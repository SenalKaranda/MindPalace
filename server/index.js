// File: server/index.js
const fastify = require('fastify')({ logger: true });
const Database = require('better-sqlite3');
const ical = require('ical-generator');
const path = require('path');
const fs = require('fs').promises;
const multipart = require('@fastify/multipart');
const crypto = require('crypto');
require('dotenv').config();

// NEW: Import axios for HTTP requests and ical.js for parsing
const axios = require('axios');
const ICAL = require('ical.js');
// For widget upload and registry
const widgetRegistryPath = path.join(__dirname, 'widgets_registry.json');

// Photo cache directory
const photoCacheDir = path.join(__dirname, 'cache', 'photos');

// Ensure cache directory exists
async function ensureCacheDir() {
  try {
    await fs.mkdir(photoCacheDir, { recursive: true });
  } catch (error) {
    console.error('Error creating cache directory:', error);
  }
}

// Get cache path for a photo
function getCachePath(sourceId, albumId, assetId, size) {
  const albumDir = albumId || 'all';
  return path.join(photoCacheDir, String(sourceId), albumDir, `${assetId}_${size}`);
}

// Clean cache for a source/album
async function cleanSourceCache(sourceId, albumId = null) {
  try {
    const sourceDir = path.join(photoCacheDir, String(sourceId));
    if (albumId) {
      // Clean specific album
      const albumDir = path.join(sourceDir, albumId);
      await fs.rm(albumDir, { recursive: true, force: true });
    } else {
      // Clean entire source
      await fs.rm(sourceDir, { recursive: true, force: true });
    }
    console.log(`[CACHE] Cleaned cache for source ${sourceId}${albumId ? `, album ${albumId}` : ''}`);
  } catch (error) {
    console.error(`[CACHE] Error cleaning cache:`, error);
  }
}

// Initialize cache directory on startup
ensureCacheDir();

// GitHub API configuration
const GITHUB_REPO_OWNER = 'jherforth';
const GITHUB_REPO_NAME = 'HomeGlowPlugins';
const GITHUB_API_BASE = 'https://api.github.com';

// Encryption utilities for calendar credentials
const ENCRYPTION_KEY = process.env.ENCRYPTION_KEY || 'homeglow-default-key-change-in-production-32bytes';
const ENCRYPTION_ALGORITHM = 'aes-256-cbc';

// Admin PIN authentication
const ADMIN_PIN = process.env.ADMIN_PIN || '1234';

function encryptPassword(password) {
  if (!password) return null;
  const key = crypto.scryptSync(ENCRYPTION_KEY, 'salt', 32);
  const iv = crypto.randomBytes(16);
  const cipher = crypto.createCipheriv(ENCRYPTION_ALGORITHM, key, iv);
  let encrypted = cipher.update(password, 'utf8', 'hex');
  encrypted += cipher.final('hex');
  return iv.toString('hex') + ':' + encrypted;
}

function decryptPassword(encryptedPassword) {
  if (!encryptedPassword) return null;
  try {
    const key = crypto.scryptSync(ENCRYPTION_KEY, 'salt', 32);
    const parts = encryptedPassword.split(':');
    const iv = Buffer.from(parts[0], 'hex');
    const encrypted = parts[1];
    const decipher = crypto.createDecipheriv(ENCRYPTION_ALGORITHM, key, iv);
    let decrypted = decipher.update(encrypted, 'hex', 'utf8');
    decrypted += decipher.final('utf8');
    return decrypted;
  } catch (error) {
    console.error('Error decrypting password:', error);
    return null;
  }
}

// Initialize Fastify with CORS
fastify.register(require('@fastify/cors'), {
  origin: '*', // Allow all origins for development. Consider restricting in production.
  methods: ['GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'OPTIONS', 'HEAD'], // All HTTP methods
  allowedHeaders: [
    'Content-Type', 
    'Authorization',
    'X-Requested-With',
    'Accept',
    'Origin',
    'Access-Control-Request-Method',
    'Access-Control-Request-Headers'
  ], // Allow all common headers
  exposedHeaders: ['Content-Type', 'Authorization'], // Headers client can read
  credentials: false, // Set to false when using origin: '*'
  maxAge: 86400 // Cache preflight requests for 24 hours
});

fastify.register(multipart);

// Add a preHandler hook to log all incoming requests
fastify.addHook('preHandler', (request, reply, done) => {
  console.log(`[${new Date().toISOString()}] ${request.method} ${request.url}`);
  if (request.body && Object.keys(request.body).length > 0) {
    // Log request body but mask sensitive data
    const bodyToLog = { ...request.body };
    if (bodyToLog.value && bodyToLog.key === 'WEATHER_API_KEY') {
      bodyToLog.value = bodyToLog.value ? `[${bodyToLog.value.length} chars]` : '[empty]';
    }
    if (bodyToLog.password) {
      bodyToLog.password = '[REDACTED]';
    }
    console.log('Request body:', JSON.stringify(bodyToLog));
  }
  done();
});

// Add error handler to log all errors
fastify.setErrorHandler((error, request, reply) => {
  console.error('=== UNHANDLED ERROR ===');
  console.error('Error:', error);
  console.error('Error message:', error.message);
  console.error('Error stack:', error.stack);
  console.error('Request URL:', request.url);
  console.error('Request method:', request.method);
  console.error('Request body:', request.body);
  reply.status(error.statusCode || 500).send({ 
    error: error.message || 'Internal server error',
    details: process.env.NODE_ENV === 'development' ? error.stack : undefined
  });
});

// Serve static files for uploads
fastify.register(require('@fastify/static'), {
  root: path.join(__dirname, 'uploads'),
  prefix: '/Uploads/',
  decorateReply: false,
  maxAge: 86400000, // 1 day cache
  setHeaders: (res, path) => {
    // Minimize headers to avoid "Request Header Fields Too Large" error
    res.setHeader('Cache-Control', 'public, max-age=86400');
  }
});

// Additional static route specifically for user uploads
fastify.register(require('@fastify/static'), {
  root: path.join(__dirname, 'uploads', 'users'),
  prefix: '/Uploads/users/',
  decorateReply: false,
  maxAge: 86400000, // 1 day cache
  setHeaders: (res, path) => {
    // Minimize headers to avoid "Request Header Fields Too Large" error
    res.setHeader('Cache-Control', 'public, max-age=86400');
  }
});

// Serve static files for widgets
fastify.register(require('@fastify/static'), {
  root: path.join(__dirname, 'widgets'),
  prefix: '/widgets/',
  decorateReply: false
});

// Add debugging for widget file requests
fastify.get('/widgets/:filename', async (request, reply) => {
  const { filename } = request.params;
  const filePath = path.join(__dirname, 'widgets', filename);
  
  console.log(`Widget request for: ${filename}`);
  console.log(`Looking for file at: ${filePath}`);
  
  try {
    const stats = await fs.stat(filePath);
    console.log(`File exists, size: ${stats.size} bytes`);
    
    const content = await fs.readFile(filePath, 'utf-8');
    console.log(`File content preview: ${content.substring(0, 100)}...`);
    
    reply.header('Content-Type', 'text/html');
    return content;
  } catch (error) {
    console.error(`Error serving widget ${filename}:`, error);
    reply.status(404).send(`Widget file not found: ${filename}`);
  }
});

// Add a simple test endpoint
fastify.get('/api/test', async (request, reply) => {
  return { 
    message: 'Server is working!', 
    timestamp: new Date().toISOString(),
    widgetsDir: path.join(__dirname, 'widgets')
  };
});

// Serve the main CSS file for widgets
fastify.get('/index.css', async (request, reply) => {
  try {
    // Try multiple possible paths
    const possiblePaths = [
      path.join(__dirname, '..', 'client', 'src', 'index.css'),
      path.join(__dirname, 'client', 'src', 'index.css'),
      '/app/client/src/index.css',
      path.join(process.cwd(), 'client', 'src', 'index.css')
    ];
    
    console.log('Looking for CSS file in paths:', possiblePaths);
    console.log('Current working directory:', process.cwd());
    console.log('__dirname:', __dirname);
    
    let cssContent = null;
    let successPath = null;
    
    for (const cssPath of possiblePaths) {
      try {
        cssContent = await fs.readFile(cssPath, 'utf-8');
        successPath = cssPath;
        console.log('Successfully found CSS at:', cssPath);
        break;
      } catch (pathError) {
        console.log('Failed to read CSS from:', cssPath, pathError.message);
      }
    }
    
    if (cssContent) {
      reply.header('Content-Type', 'text/css');
      reply.header('Access-Control-Allow-Origin', '*');
      return cssContent;
    }
    
    throw new Error('CSS file not found in any expected location');
  } catch (error) {
    console.error('Error serving index.css:', error);
    
    // Fallback: serve minimal CSS for widgets
    const fallbackCSS = `
      :root {
        --background: #f4f4f9;
        --card-bg: rgba(255, 255, 255, 0.8);
        --card-border: rgba(255, 255, 255, 0.2);
        --text-color: #1a1a2e;
        --text-color-rgb: 26, 26, 46;
        --accent: #6e44ff;
        --accent-rgb: 110, 68, 255;
        --shadow: 0 8px 32px rgba(0, 0, 0, 0.1);
        --backdrop-blur: blur(10px);
        --dynamic-text-size: 16px;
        --dynamic-card-width: 300px;
        --dynamic-card-padding: 20px;
        --error-color: #ff4444;
        --light-gradient-start: #00ddeb;
        --light-gradient-end: #ff6b6b;
        --dark-gradient-start: #2e2767;
        --dark-gradient-end: #620808;
        --light-button-gradient-start: #00ddeb;
        --light-button-gradient-end: #ff6b6b;
        --dark-button-gradient-start: #2e2767;
        --dark-button-gradient-end: #620808;
        --gradient: linear-gradient(45deg, var(--light-gradient-start), var(--light-gradient-end));
      }
      
      [data-theme="dark"] {
        --background: #0a0a1a;
        --card-bg: rgba(30, 30, 50, 0.7);
        --card-border: rgba(100, 100, 150, 0.3);
        --text-color: #a6a6d1;
        --text-color-rgb: 166, 166, 209;
        --accent: #00ddeb;
        --accent-rgb: 0, 221, 235;
        --shadow: 0 8px 32px rgba(0, 0, 0, 0.3);
        --gradient: linear-gradient(45deg, var(--dark-gradient-start), var(--dark-gradient-end));
      }
      
      html, body {
        margin: 0;
        font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
        background: var(--background);
        color: var(--text-color);
        transition: background 0.3s ease, color 0.3s ease;
        touch-action: manipulation;
        width: 100%;
        height: 100%;
        overflow-x: hidden;
        overflow-y: auto;
        font-size: var(--dynamic-text-size);
      }
      
      .card {
        background: var(--card-bg);
        border: 1px solid var(--card-border);
        border-radius: 12px;
        padding: var(--dynamic-card-padding);
        backdrop-filter: var(--backdrop-blur);
        box-shadow: var(--shadow);
        transition: transform 0.2s ease, box-shadow 0.2s ease;
        width: 100%;
        max-width: var(--dynamic-card-width);
        touch-action: manipulation;
      }
      
      .card:hover {
        transform: translateY(-5px);
        box-shadow: 0 12px 40px rgba(0, 0, 0, 0.2);
      }
      
      h1, h2, h3, h4, h5, h6 {
        font-weight: 700;
        letter-spacing: 0.5px;
        color: var(--text-color);
      }
      
      button {
        background: linear-gradient(45deg, var(--light-button-gradient-start), var(--light-button-gradient-end));
        color: var(--text-color);
        border: none;
        border-radius: 8px;
        padding: 10px 20px;
        cursor: pointer;
        font-size: 1rem;
        font-weight: 600;
        transition: background 0.3s ease;
        touch-action: manipulation;
      }
      
      [data-theme="dark"] button {
        background: linear-gradient(45deg, var(--dark-button-gradient-start), var(--dark-button-gradient-end));
      }
      
      button:hover {
        filter: brightness(1.1);
      }
    `;
    
    console.log('Serving fallback CSS');
    reply.header('Content-Type', 'text/css');
    reply.header('Access-Control-Allow-Origin', '*');
    return fallbackCSS;
  }
});

// --- Widget Upload Endpoint and Registry ---

// Helper: Load widget registry
async function loadWidgetRegistry() {
  try {
    const data = await fs.readFile(widgetRegistryPath, 'utf-8');
    return JSON.parse(data);
  } catch (err) {
    return [];
  }
}

// Helper: Save widget registry
async function saveWidgetRegistry(registry) {
  await fs.writeFile(widgetRegistryPath, JSON.stringify(registry, null, 2), 'utf-8');
}

// Endpoint: Upload a widget (HTML file)
fastify.post('/api/widgets/upload', async (request, reply) => {
  try {
    const data = await request.file();
    if (!data || !data.filename.endsWith('.html')) {
      return reply.status(400).send({ error: 'Only HTML widget files are allowed.' });
    }

    const widgetName = data.filename.replace(/[^a-zA-Z0-9-._]/g, '_');
    const savePath = path.join(__dirname, 'widgets', widgetName);

    // Save the file
    await fs.writeFile(savePath, await data.toBuffer());

    // Update registry
    const registry = await loadWidgetRegistry();
    if (!registry.find(w => w.filename === widgetName)) {
      registry.push({
        name: widgetName.replace('.html', ''),
        filename: widgetName,
        uploadedAt: new Date().toISOString()
      });
      await saveWidgetRegistry(registry);
    }

    return { success: true, message: 'Widget uploaded!', widget: widgetName };
  } catch (err) {
    console.error('Widget upload error:', err);
    reply.status(500).send({ error: 'Failed to upload widget.' });
  }
});

// Endpoint: List widgets
fastify.get('/api/widgets', async (request, reply) => {
  try {
    const registry = await loadWidgetRegistry();
    return registry;
  } catch (err) {
    reply.status(500).send({ error: 'Failed to load widget registry.' });
  }
});

// Endpoint: Delete a widget
fastify.delete('/api/widgets/:filename', async (request, reply) => {
  const { filename } = request.params;
  try {
    const filePath = path.join(__dirname, 'widgets', filename);
    await fs.unlink(filePath);

    // Update registry
    let registry = await loadWidgetRegistry();
    registry = registry.filter(w => w.filename !== filename);
    await saveWidgetRegistry(registry);

    return { success: true, message: 'Widget deleted.' };
  } catch (err) {
    reply.status(500).send({ error: 'Failed to delete widget.' });
  }
});

// Debug endpoint to list widget files
fastify.get('/api/widgets/debug', async (request, reply) => {
  try {
    const widgetsDir = path.join(__dirname, 'widgets');
    console.log(`Checking widgets directory: ${widgetsDir}`);
    
    const files = await fs.readdir(widgetsDir);
    console.log(`Files in widgets directory:`, files);
    
    const fileDetails = [];
    for (const file of files) {
      const filePath = path.join(widgetsDir, file);
      try {
        const stats = await fs.stat(filePath);
        fileDetails.push({
          name: file,
          size: stats.size,
          isFile: stats.isFile(),
          modified: stats.mtime
        });
      } catch (err) {
        fileDetails.push({
          name: file,
          error: err.message
        });
      }
    }
    
    return {
      directory: widgetsDir,
      files: fileDetails,
      registry: await loadWidgetRegistry()
    };
  } catch (error) {
    console.error('Error reading widgets directory:', error);
    return { error: error.message };
  }
});

// Endpoint: List available widgets from GitHub repository
fastify.get('/api/widgets/github', async (request, reply) => {
  try {
    console.log('Fetching widgets from GitHub repository...');
    
    // Get repository contents
    const repoUrl = `${GITHUB_API_BASE}/repos/${GITHUB_REPO_OWNER}/${GITHUB_REPO_NAME}/contents`;
    const response = await axios.get(repoUrl, {
      headers: {
        'Accept': 'application/vnd.github.v3+json',
        'User-Agent': 'HomeGlow-Server/1.0'
      },
      timeout: 10000
    });

    // Filter for HTML files and directories
    const items = response.data.filter(item => 
      item.type === 'file' && item.name.endsWith('.html') ||
      item.type === 'dir'
    );

    const widgets = [];
    
    for (const item of items) {
      if (item.type === 'file' && item.name.endsWith('.html')) {
        // Direct HTML file in root
        widgets.push({
          name: item.name.replace('.html', ''),
          filename: item.name,
          description: `Widget: ${item.name.replace('.html', '')}`,
          download_url: item.download_url,
          path: item.path,
          size: item.size,
          type: 'file'
        });
      } else if (item.type === 'dir') {
        // Check if directory contains HTML files
        try {
          const dirUrl = `${GITHUB_API_BASE}/repos/${GITHUB_REPO_OWNER}/${GITHUB_REPO_NAME}/contents/${item.path}`;
          const dirResponse = await axios.get(dirUrl, {
            headers: {
              'Accept': 'application/vnd.github.v3+json',
              'User-Agent': 'HomeGlow-Server/1.0'
            },
            timeout: 5000
          });
          
          const htmlFiles = dirResponse.data.filter(file => 
            file.type === 'file' && file.name.endsWith('.html')
          );
          
          for (const htmlFile of htmlFiles) {
            widgets.push({
              name: `${item.name}/${htmlFile.name.replace('.html', '')}`,
              filename: htmlFile.name,
              description: `Widget from ${item.name} folder`,
              download_url: htmlFile.download_url,
              path: htmlFile.path,
              size: htmlFile.size,
              type: 'file',
              folder: item.name
            });
          }
        } catch (dirError) {
          console.warn(`Could not read directory ${item.name}:`, dirError.message);
        }
      }
    }

    console.log(`Found ${widgets.length} widgets in GitHub repository`);
    return widgets;
    
  } catch (error) {
    console.error('Error fetching GitHub widgets:', error);
    if (error.response) {
      return reply.status(error.response.status).send({ 
        error: `GitHub API error: ${error.response.status} ${error.response.statusText}`,
        details: error.response.data 
      });
    }
    return reply.status(500).send({ 
      error: 'Failed to fetch widgets from GitHub repository',
      details: error.message 
    });
  }
});

// Endpoint: Install a widget from GitHub repository
fastify.post('/api/widgets/github/install', async (request, reply) => {
  try {
    const { download_url, filename, name } = request.body;
    
    if (!download_url || !filename) {
      return reply.status(400).send({ error: 'download_url and filename are required' });
    }

    console.log(`Installing widget ${filename} from GitHub...`);
    
    // Download the widget file
    const response = await axios.get(download_url, {
      headers: {
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8',
        'User-Agent': 'HomeGlow-Server/1.0'
      },
      timeout: 15000
    });
    
    // Sanitize filename
    const sanitizedFilename = filename.replace(/[^a-zA-Z0-9-._]/g, '_');
    const savePath = path.join(__dirname, 'widgets', sanitizedFilename);
    
    // Save the widget file
    await fs.writeFile(savePath, response.data, 'utf-8');
    
    // Update registry
    const registry = await loadWidgetRegistry();
    const existingWidget = registry.find(w => w.filename === sanitizedFilename);
    
    if (!existingWidget) {
      registry.push({
        name: name || sanitizedFilename.replace('.html', ''),
        filename: sanitizedFilename,
        uploadedAt: new Date().toISOString(),
        source: 'github',
        originalUrl: download_url
      });
      await saveWidgetRegistry(registry);
    }
    
    console.log(`Successfully installed widget: ${sanitizedFilename}`);
    return { 
      success: true, 
      message: 'Widget installed successfully!', 
      widget: sanitizedFilename 
    };
    
  } catch (error) {
    console.error('Error installing GitHub widget:', error);
    if (error.response) {
      return reply.status(error.response.status).send({ 
        error: `Failed to download widget: ${error.response.status} ${error.response.statusText}`,
        details: error.response.data 
      });
    }
    return reply.status(500).send({ 
      error: 'Failed to install widget from GitHub',
      details: error.message 
    });
  }
});

// Initialize database
const dbPath = path.resolve(__dirname, 'data', 'tasks.db');
console.log('Database path:', dbPath);
let db; // Declare db variable outside to hold the single instance

async function initializeDatabase() {
  try {
    // Ensure the 'data' directory exists and is writable
    await fs.mkdir(path.dirname(dbPath), { recursive: true });
    await fs.chmod(path.dirname(dbPath), 0o777); // Ensure directory is writable
    const newDb = new Database(dbPath, { verbose: console.log });
    newDb.exec(`
      CREATE TABLE IF NOT EXISTS chores (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id INTEGER,
        title TEXT,\
        description TEXT,\
        time_period TEXT,\
        assigned_day_of_week TEXT,\
        repeat_type TEXT,\
        completed BOOLEAN,
        clam_value INTEGER DEFAULT 0,
        expiration_date TEXT
      );\
      CREATE TABLE IF NOT EXISTS users (
        id INTEGER PRIMARY KEY AUTOINCREMENT,\
        username TEXT,\
        email TEXT,\
        profile_picture TEXT,\
        clam_total INTEGER DEFAULT 0
      );
      INSERT OR IGNORE INTO users (id, username, email, profile_picture, clam_total) VALUES (0, 'bonus', 'bonus@example.com', '', 0);\
      CREATE TABLE IF NOT EXISTS events (
        id INTEGER PRIMARY KEY AUTOINCREMENT,\
        user_id INTEGER,\
        summary TEXT,\
        start TEXT,\
        end TEXT,\
        description TEXT
      );\
      CREATE TABLE IF NOT EXISTS settings (
        key TEXT PRIMARY KEY,
        value TEXT
      );
      CREATE TABLE IF NOT EXISTS prizes (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        name TEXT NOT NULL,
        clam_cost INTEGER NOT NULL
      );
      CREATE TABLE IF NOT EXISTS calendar_sources (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        name TEXT NOT NULL,
        type TEXT NOT NULL,
        url TEXT NOT NULL,
        username TEXT,
        password TEXT,
        color TEXT NOT NULL DEFAULT '#6e44ff',
        enabled INTEGER NOT NULL DEFAULT 1,
        sort_order INTEGER NOT NULL DEFAULT 0,
        created_at TEXT DEFAULT CURRENT_TIMESTAMP
      );
      CREATE INDEX IF NOT EXISTS idx_calendar_sources_enabled ON calendar_sources(enabled);
      CREATE INDEX IF NOT EXISTS idx_calendar_sources_sort_order ON calendar_sources(sort_order);
      CREATE TABLE IF NOT EXISTS photo_sources (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        name TEXT NOT NULL,
        type TEXT NOT NULL,
        url TEXT,
        api_key TEXT,
        username TEXT,
        password TEXT,
        album_id TEXT,
        refresh_token TEXT,
        enabled INTEGER NOT NULL DEFAULT 1,
        sort_order INTEGER NOT NULL DEFAULT 0,
        created_at TEXT DEFAULT CURRENT_TIMESTAMP
      );
      CREATE INDEX IF NOT EXISTS idx_photo_sources_enabled ON photo_sources(enabled);
      CREATE INDEX IF NOT EXISTS idx_photo_sources_sort_order ON photo_sources(sort_order);
      CREATE TABLE IF NOT EXISTS alarms (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        title TEXT NOT NULL,
        time TEXT NOT NULL,
        repeat_type TEXT NOT NULL DEFAULT 'none',
        repeat_days TEXT,
        enabled INTEGER NOT NULL DEFAULT 1,
        sound_enabled INTEGER NOT NULL DEFAULT 1,
        notification_enabled INTEGER NOT NULL DEFAULT 1,
        visual_alert_enabled INTEGER NOT NULL DEFAULT 1,
        last_triggered TEXT,
        created_at TEXT DEFAULT CURRENT_TIMESTAMP
      );
      CREATE INDEX IF NOT EXISTS idx_alarms_enabled ON alarms(enabled);
      CREATE TABLE IF NOT EXISTS house_rules (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        rule_text TEXT NOT NULL,
        order_index INTEGER NOT NULL DEFAULT 0,
        created_at TEXT DEFAULT CURRENT_TIMESTAMP,
        updated_at TEXT DEFAULT CURRENT_TIMESTAMP
      );
      CREATE INDEX IF NOT EXISTS idx_house_rules_order ON house_rules(order_index);
      CREATE TABLE IF NOT EXISTS marbles (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id INTEGER NOT NULL,
        count INTEGER NOT NULL DEFAULT 0,
        track_marbles INTEGER NOT NULL DEFAULT 0,
        created_at TEXT DEFAULT CURRENT_TIMESTAMP,
        updated_at TEXT DEFAULT CURRENT_TIMESTAMP,
        UNIQUE(user_id)
      );
      CREATE INDEX IF NOT EXISTS idx_marbles_user_id ON marbles(user_id);
      CREATE TABLE IF NOT EXISTS marble_history (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id INTEGER NOT NULL,
        change_amount INTEGER NOT NULL,
        change_type TEXT NOT NULL,
        reason TEXT,
        created_at TEXT DEFAULT CURRENT_TIMESTAMP
      );
      CREATE INDEX IF NOT EXISTS idx_marble_history_user_id ON marble_history(user_id);
      CREATE INDEX IF NOT EXISTS idx_marble_history_created_at ON marble_history(created_at);
      CREATE TABLE IF NOT EXISTS meals (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        title TEXT NOT NULL,
        description TEXT,
        recipe TEXT,
        recipe_url TEXT,
        cooktime INTEGER,
        preptime INTEGER,
        servings INTEGER,
        category TEXT,
        ingredients TEXT NOT NULL,
        date_specific INTEGER DEFAULT 0,
        assigned_date TEXT,
        assigned_weekday TEXT,
        created_at TEXT DEFAULT CURRENT_TIMESTAMP,
        updated_at TEXT DEFAULT CURRENT_TIMESTAMP
      );
      CREATE INDEX IF NOT EXISTS idx_meals_date ON meals(assigned_date);
      CREATE INDEX IF NOT EXISTS idx_meals_weekday ON meals(assigned_weekday);
      CREATE TABLE IF NOT EXISTS grocery_items (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        name TEXT NOT NULL,
        quantity TEXT,
        unit TEXT,
        user_id INTEGER,
        meal_ids TEXT,
        checked INTEGER DEFAULT 0,
        checked_by INTEGER,
        created_at TEXT DEFAULT CURRENT_TIMESTAMP,
        updated_at TEXT DEFAULT CURRENT_TIMESTAMP
      );
      CREATE INDEX IF NOT EXISTS idx_grocery_items_user ON grocery_items(user_id);
      CREATE INDEX IF NOT EXISTS idx_grocery_items_checked ON grocery_items(checked);
      CREATE TABLE IF NOT EXISTS meal_suggestions (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id INTEGER NOT NULL,
        suggestion_text TEXT NOT NULL,
        created_at TEXT DEFAULT CURRENT_TIMESTAMP
      );
      CREATE INDEX IF NOT EXISTS idx_meal_suggestions_user ON meal_suggestions(user_id);
      CREATE INDEX IF NOT EXISTS idx_meal_suggestions_created ON meal_suggestions(created_at);
    `);
    
    // Set default MARBLE_DAILY_INCREMENT if not exists
    const marbleIncrement = newDb.prepare('SELECT value FROM settings WHERE key = ?').get('MARBLE_DAILY_INCREMENT');
    if (!marbleIncrement) {
      newDb.prepare('INSERT INTO settings (key, value) VALUES (?, ?)').run('MARBLE_DAILY_INCREMENT', '3');
    }
    
    // Set default PRIZE_MINIMUM_SHELLS if not exists
    const prizeMinimum = newDb.prepare('SELECT value FROM settings WHERE key = ?').get('PRIZE_MINIMUM_SHELLS');
    if (!prizeMinimum) {
      newDb.prepare('INSERT INTO settings (key, value) VALUES (?, ?)').run('PRIZE_MINIMUM_SHELLS', '0');
    }
    
    // Migration: Add track_marbles column if it doesn't exist
    try {
      const columns = newDb.prepare("PRAGMA table_info(marbles)").all();
      const hasTrackMarbles = columns.some(col => col.name === 'track_marbles');
      
      if (!hasTrackMarbles) {
        console.log('Adding track_marbles column to marbles table...');
        newDb.exec('ALTER TABLE marbles ADD COLUMN track_marbles INTEGER NOT NULL DEFAULT 0');
        // Existing marbles should not be tracked by default
        console.log('Existing marbles set to not tracked by default. Enable tracking in admin panel.');
      }
    } catch (migrationError) {
      console.error('Migration error adding track_marbles:', migrationError);
    }
    
    // Migration: Add repeat_type column if it doesn't exist and remove old repeats column
    try {
      // Check if repeat_type column exists
      const columns = newDb.prepare("PRAGMA table_info(chores)").all();
      const hasRepeatType = columns.some(col => col.name === 'repeat_type');
      const hasRepeats = columns.some(col => col.name === 'repeats');
      
      if (!hasRepeatType) {
        console.log('Adding repeat_type column to chores table...');
        newDb.exec('ALTER TABLE chores ADD COLUMN repeat_type TEXT DEFAULT "no-repeat"');
        
        // If old repeats column exists, migrate data
        if (hasRepeats) {
          console.log('Migrating data from repeats to repeat_type...');
          newDb.exec(`
            UPDATE chores 
            SET repeat_type = CASE 
              WHEN repeats = 1 OR repeats = 'true' THEN 'weekly'
              ELSE 'no-repeat'
            END
          `);
        }
      }
      
      // Remove old repeats column if it exists
      if (hasRepeats) {
        console.log('Removing old repeats column...');
        // SQLite doesn't support DROP COLUMN directly, so we need to recreate the table
        newDb.exec(`
          CREATE TABLE chores_new (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id INTEGER,
            title TEXT,
            description TEXT,
            time_period TEXT,
            assigned_day_of_week TEXT,
            repeat_type TEXT,
            completed BOOLEAN,
            clam_value INTEGER DEFAULT 0,
            expiration_date TEXT
          );
          
          INSERT INTO chores_new (id, user_id, title, description, time_period, assigned_day_of_week, repeat_type, completed, clam_value, expiration_date)
          SELECT id, user_id, title, description, time_period, assigned_day_of_week, repeat_type, completed, clam_value, expiration_date
          FROM chores;
          
          DROP TABLE chores;
          ALTER TABLE chores_new RENAME TO chores;
        `);
        console.log('Migration completed successfully!');
      }
      
    } catch (migrationError) {
      console.error('Migration error:', migrationError);
      // Continue anyway - the app might still work with the new schema
    }

    // Migration: Note that ICS is no longer supported
    // If ICS_CALENDAR_URL exists, user should manually add it as CalDAV calendar
    try {
      const icsUrlSetting = newDb.prepare('SELECT value FROM settings WHERE key = ?').get('ICS_CALENDAR_URL');
      if (icsUrlSetting && icsUrlSetting.value) {
        console.log('Note: ICS_CALENDAR_URL found but ICS is no longer supported. User should add as CalDAV calendar.');
      }
    } catch (calendarMigrationError) {
      console.error('Calendar migration error:', calendarMigrationError);
    }

    return newDb; // Return the new database instance
  } catch (error) {
    console.error('Failed to initialize database:', error);
    throw error;
  }
}

// Function to prune and reset chores based on the day
async function pruneAndResetChores() {
  try {
    const days = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'];
    const currentDay = days[new Date().getDay()];
    const now = new Date();

    // Select all chores to process
    const allChores = db.prepare('SELECT id, user_id, assigned_day_of_week, repeat_type, completed, clam_value, expiration_date FROM chores').all();

    for (const chore of allChores) {
      // Handle bonus chores
      if (chore.clam_value > 0) {
        // If bonus chore is completed, delete it
        if (chore.completed) {
          db.prepare('DELETE FROM chores WHERE id = ?').run(chore.id);
          console.log(`Deleted completed bonus chore ID ${chore.id}.`);
        } else if (chore.expiration_date) {
          // If bonus chore is not completed and has an expiration date
          const expirationDate = new Date(chore.expiration_date);
          if (now > expirationDate) {
            // Reassign to bonus user (ID 0) and reset completed status
            db.prepare('UPDATE chores SET user_id = 0, completed = 0, expiration_date = NULL WHERE id = ?').run(chore.id);
            console.log(`Reassigned expired bonus chore ID ${chore.id} back to bonus user.`);
          }
        }
      } else {
        // Handle regular chores (existing logic)
        if (chore.completed) {
          if (chore.repeat_type === "no-repeat") {
            // Only delete non-repeating chores if they're not for today
            if (chore.assigned_day_of_week !== currentDay) {
              db.prepare('DELETE FROM chores WHERE id = ?').run(chore.id);
              console.log(`Deleted non-repeating chore ID ${chore.id}.`);
            }
          } else if (chore.repeat_type === "until-completed") {
            // Delete "until-completed" chores once they're completed
            db.prepare('DELETE FROM chores WHERE id = ?').run(chore.id);
            console.log(`Deleted "until-completed" chore ID ${chore.id} after completion.`);
          } else if (chore.repeat_type === "weekly" || chore.repeat_type === "daily") {
            // For daily chores, reset every day. For weekly chores, reset when it's their assigned day again
            if (chore.repeat_type === "daily" || chore.assigned_day_of_week === currentDay) {
              db.prepare('UPDATE chores SET completed = 0 WHERE id = ?').run(chore.id);
              console.log(`Reset repeating chore ID ${chore.id} to uncompleted.`);
            }
          }
        }
        
        // Handle "until-completed" chores that need to appear on new days
        if (!chore.completed && chore.repeat_type === "until-completed") {
          // For "until-completed" chores that aren't completed, check if they should repeat daily
          if (chore.assigned_day_of_week !== currentDay) {
            // Create a new instance for today if it doesn't exist
            const existingTodayChore = db.prepare('SELECT id FROM chores WHERE user_id = ? AND title = ? AND assigned_day_of_week = ? AND repeat_type = "until-completed"').get(chore.user_id, chore.title, currentDay);
            if (!existingTodayChore) {
              db.prepare('INSERT INTO chores (user_id, title, description, time_period, assigned_day_of_week, repeat_type, completed, clam_value, expiration_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(
                chore.user_id, chore.title, chore.description || '', chore.time_period || 'any-time', currentDay, 'until-completed', 0, 0, null
              );
              console.log(`Created new "until-completed" chore instance for today: ${chore.title}`);
            }
          }
        }
      }
    }
  } catch (error) {
    console.error('Error during chore pruning and reset:', error);
  }
}


// Chore routes
fastify.get('/api/chores', async (request, reply) => {
  try {
    const rows = db.prepare('SELECT * FROM chores').all(); // Use the global db instance
    return rows;
  } catch (error) {
    console.error('Error fetching chores:', error);
    reply.status(500).send({ error: 'Failed to fetch chores' });
  }
});

fastify.post('/api/chores', async (request, reply) => {
  const { user_id, title, description, time_period, assigned_day_of_week, repeat_type, completed, clam_value, expiration_date } = request.body;
  try {
    const completedInt = completed ? 1 : 0;
    const stmt = db.prepare('INSERT INTO chores (user_id, title, description, time_period, assigned_day_of_week, repeat_type, completed, clam_value, expiration_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)');
    const info = stmt.run(user_id, title, description, time_period, assigned_day_of_week, repeat_type, completedInt, clam_value, expiration_date);
    return { id: info.lastInsertRowid };
  } catch (error) {
    console.error('Error adding chore:', error);
    reply.status(500).send({ error: 'Failed to add chore' });
  }
});

fastify.patch('/api/chores/:id', async (request, reply) => {
  const { id } = request.params;
  const { completed } = request.body;
  try {
    const completedInt = completed ? 1 : 0;
    const stmt = db.prepare('UPDATE chores SET completed = ? WHERE id = ?');
    stmt.run(completedInt, id);

    // --- Clam Reward Logic ---\
    // Get the chore details to find the user_id and assigned_day_of_week
    const chore = db.prepare('SELECT user_id, clam_value, assigned_day_of_week FROM chores WHERE id = ?').get(id);

    if (chore) {
      // 1. Reward for bonus chores
      if (completed && chore.clam_value > 0) { // Only reward if marked completed and it's a bonus chore
        const userUpdateStmt = db.prepare('UPDATE users SET clam_total = clam_total + ? WHERE id = ?');
        userUpdateStmt.run(chore.clam_value, chore.user_id);
        console.log(`User ${chore.user_id} rewarded ${chore.clam_value} clams for completing bonus chore ID ${id}.`);
      }

      // 2. Reward for completing all *regular* daily chores
      // Only apply this if the current chore is NOT a bonus chore (clam_value === 0)
      if (completed && chore.clam_value === 0) {
        // Get all *regular* chores for this user and day
        const usersRegularChoresForDay = db.prepare('SELECT completed FROM chores WHERE user_id = ? AND assigned_day_of_week = ? AND clam_value = 0').all(chore.user_id, chore.assigned_day_of_week);

        // Check if all *regular* chores for this user and day are completed
        const allRegularChoresCompleted = usersRegularChoresForDay.every(c => c.completed === 1);

        if (allRegularChoresCompleted) {
          // Reward user with 2 clams
          const userUpdateStmt = db.prepare('UPDATE users SET clam_total = clam_total + 2 WHERE id = ?');
          userUpdateStmt.run(chore.user_id);
          console.log(`User ${chore.user_id} rewarded 2 clams for completing all regular chores on ${chore.assigned_day_of_week}.`);
        }
      }
    }
    // --- End Clam Reward Logic ---\

    return { success: true };
  } catch (error) {
    console.error('Error updating chore:', error);
    reply.status(500).send({ error: 'Failed to update chore' });
  }
});

// NEW: Endpoint to delete a chore
fastify.delete('/api/chores/:id', async (request, reply) => {
  const { id } = request.params;
  try {
    const stmt = db.prepare('DELETE FROM chores WHERE id = ?');
    const info = stmt.run(id);
    if (info.changes === 0) {
      return reply.status(404).send({ error: 'Chore not found' });
    }
    return { success: true, message: 'Chore deleted successfully' };
  } catch (error) {
    console.error('Error deleting chore:', error);
    reply.status(500).send({ error: 'Failed to delete chore' });
  }
});


// NEW: Endpoint to assign a bonus chore to a user
fastify.patch('/api/chores/:id/assign', async (request, reply) => {
  const { id } = request.params;
  const { user_id } = request.body;

  try {
    // 1. Check if the chore exists and is a bonus chore (assigned to user_id 0)
    const chore = db.prepare('SELECT id, user_id, completed, clam_value FROM chores WHERE id = ?').get(id);
    if (!chore) {
      return reply.status(404).send({ error: 'Chore not found.' });
    }
    if (chore.user_id !== 0 || chore.clam_value === 0) {
      return reply.status(400).send({ error: 'This is not an unassigned bonus chore.' });
    }

    // 2. Check if the target user already has an uncompleted bonus chore
    const existingBonusChore = db.prepare('SELECT id FROM chores WHERE user_id = ? AND clam_value > 0 AND completed = 0').get(user_id);
    if (existingBonusChore) {
      return reply.status(409).send({ error: 'User already has an uncompleted bonus chore. Complete it first!' });
    }

    // 3. Assign the chore to the user and set expiration date (24 hours from now)
    const expirationDate = new Date();
    expirationDate.setHours(expirationDate.getHours() + 24);

    const stmt = db.prepare('UPDATE chores SET user_id = ?, completed = 0, expiration_date = ? WHERE id = ?');
    stmt.run(user_id, expirationDate.toISOString(), id);

    return { success: true, message: 'Bonus chore assigned successfully.' };
  } catch (error) {
    console.error('Error assigning bonus chore:', error);
    reply.status(500).send({ error: 'Failed to assign bonus chore.' });
  }
});


// User routes
fastify.get('/api/users', async (request, reply) => {
  try {
    const rows = db.prepare('SELECT id, username, email, profile_picture, clam_total FROM users').all();
    return rows;
  } catch (error) {
    console.error('Error fetching users:', error);
    reply.status(500).send({ error: 'Failed to fetch users' });
  }
});

fastify.post('/api/users', async (request, reply) => {
  try {
    const { username, email, profile_picture } = request.body;
    console.log('=== ADDING USER ===');
    console.log('Request body:', JSON.stringify(request.body));
    console.log('Username:', username);
    console.log('Email:', email);
    
    if (!username || !email) {
      console.error('ERROR: Missing required fields');
      return reply.status(400).send({ error: 'Username and email are required.' });
    }
    
    // Check if database is initialized
    if (!db) {
      console.error('ERROR: Database not initialized');
      return reply.status(500).send({ error: 'Database not initialized. Please restart the server.' });
    }
    
    try {
      const stmt = db.prepare('INSERT INTO users (username, email, profile_picture) VALUES (?, ?, ?)');
      const info = stmt.run(username, email, profile_picture || '');
      console.log('User added successfully with ID:', info.lastInsertRowid);
      return { id: info.lastInsertRowid, success: true, message: 'User added successfully' };
    } catch (dbError) {
      console.error('Database error adding user:', dbError);
      console.error('Database error stack:', dbError.stack);
      
      // Check for unique constraint violation
      if (dbError.message && dbError.message.includes('UNIQUE constraint')) {
        return reply.status(409).send({ error: 'User with this username or email already exists.' });
      }
      
      return reply.status(500).send({ 
        error: 'Failed to add user',
        details: dbError.message 
      });
    }
  } catch (error) {
    console.error('Unexpected error in /api/users:', error);
    console.error('Error stack:', error.stack);
    return reply.status(500).send({ 
      error: 'Unexpected error occurred',
      details: error.message 
    });
  }
});

// NEW: Endpoint to update user profile
fastify.patch('/api/users/:id', async (request, reply) => {
  const { id } = request.params;
  const { username, email, profile_picture } = request.body;
  try {
    const stmt = db.prepare('UPDATE users SET username = ?, email = ?, profile_picture = ? WHERE id = ?');
    const info = stmt.run(username, email, profile_picture, id);
    if (info.changes === 0) {
      return reply.status(404).send({ error: 'User not found' });
    }
    return { success: true, message: 'User updated successfully' };
  } catch (error) {
    console.error('Error updating user:', error);
    reply.status(500).send({ error: 'Failed to update user' });
  }
});

// NEW: Endpoint to upload user profile picture
fastify.post('/api/users/:id/upload-picture', async (request, reply) => {
  try {
    const { id } = request.params;
    const data = await request.file();
    
    if (!data) {
      return reply.status(400).send({ error: 'No file uploaded' });
    }

    // Validate file type
    const allowedTypes = ['image/jpeg', 'image/jpg', 'image/png', 'image/gif'];
    if (!allowedTypes.includes(data.mimetype)) {
      return reply.status(400).send({ error: 'Only image files (JPEG, PNG, GIF) are allowed' });
    }

    // Create users directory if it doesn't exist
    const usersDir = path.join(__dirname, 'uploads', 'users');
    await fs.mkdir(usersDir, { recursive: true });

    // Generate unique filename
    const fileExtension = data.filename.split('.').pop();
    const filename = `user_${id}_${Date.now()}.${fileExtension}`;
    const filepath = path.join(usersDir, filename);

    // Save file
    await fs.writeFile(filepath, await data.toBuffer());

    // Update user record
    const stmt = db.prepare('UPDATE users SET profile_picture = ? WHERE id = ?');
    const info = stmt.run(filename, id);
    
    if (info.changes === 0) {
      // Clean up uploaded file if user doesn't exist
      await fs.unlink(filepath);
      return reply.status(404).send({ error: 'User not found' });
    }

    return { 
      success: true, 
      message: 'Profile picture uploaded successfully',
      filename: filename 
    };
  } catch (error) {
    console.error('Error uploading profile picture:', error);
    reply.status(500).send({ error: 'Failed to upload profile picture' });
  }
});

// NEW: Endpoint to update user clam total (for manual adjustments or future use)
fastify.patch('/api/users/:id/clams', async (request, reply) => {
  const { id } = request.params;
  const { clam_total } = request.body; // Expecting the new total or a delta
  try {
    const stmt = db.prepare('UPDATE users SET clam_total = ? WHERE id = ?');
    stmt.run(clam_total, id);
    return { success: true };
  } catch (error) {
    console.error('Error updating user clams:', error);
    reply.status(500).send({ error: 'Failed to update user clams' });
  }
});

// NEW: Endpoint to delete a user
fastify.delete('/api/users/:id', async (request, reply) => {
  try {
    const { id } = request.params;
    console.log('=== DELETING USER ===');
    console.log('User ID:', id);
    
    if (!id) {
      return reply.status(400).send({ error: 'User ID is required.' });
    }
    
    // Check if database is initialized
    if (!db) {
      console.error('ERROR: Database not initialized');
      return reply.status(500).send({ error: 'Database not initialized. Please restart the server.' });
    }
    
    try {
      const stmt = db.prepare('DELETE FROM users WHERE id = ?');
      const info = stmt.run(id);
      console.log('Delete result:', info);
      
      if (info.changes === 0) {
        console.log('User not found with ID:', id);
        return reply.status(404).send({ error: 'User not found' });
      }
      
      console.log('User deleted successfully');
      return { success: true, message: 'User deleted successfully' };
    } catch (dbError) {
      console.error('Database error deleting user:', dbError);
      console.error('Database error stack:', dbError.stack);
      return reply.status(500).send({ 
        error: 'Failed to delete user',
        details: dbError.message 
      });
    }
  } catch (error) {
    console.error('Unexpected error in DELETE /api/users/:id:', error);
    console.error('Error stack:', error.stack);
    return reply.status(500).send({ 
      error: 'Unexpected error occurred',
      details: error.message 
    });
  }
});


// Calendar routes (existing)
fastify.get('/api/calendar', async (request, reply) => {
  try {
    const rows = db.prepare('SELECT * FROM events').all();
    return rows;
  }  catch (error) {
    console.error('Error fetching events:', error);
    reply.status(500).send({ error: 'Failed to fetch events' });
  }
});

fastify.post('/api/calendar', async (request, reply) => {
  const { user_id, summary, start, end, description } = request.body;
  try {
    const stmt = db.prepare('INSERT INTO events (user_id, summary, start, end, description) VALUES (?, ?, ?, ?, ?)');
    const info = stmt.run(user_id, summary, start, end, description);
    return { id: info.lastInsertRowid };
  } catch (error) {
    console.error('Error adding event:', error);
    reply.status(500).send({ error: 'Failed to add event' });
  }
});


// PIN Authentication Helper
function requireAdminPIN(request, reply) {
  const providedPIN = request.headers['x-admin-pin'] || request.body?.pin;
  if (!providedPIN || providedPIN !== ADMIN_PIN) {
    return reply.status(401).send({ error: 'Invalid or missing admin PIN' });
  }
  return null; // PIN is valid
}

// PIN Verification Endpoint (for frontend to check PIN)
fastify.post('/api/admin/verify-pin', async (request, reply) => {
  try {
    const { pin } = request.body;
    if (pin === ADMIN_PIN) {
      return { success: true, message: 'PIN verified' };
    } else {
      return reply.status(401).send({ error: 'Invalid PIN' });
    }
  } catch (error) {
    console.error('Error verifying PIN:', error);
    return reply.status(500).send({ error: 'Failed to verify PIN' });
  }
});

// NEW: API Endpoints for Settings (including API keys)
fastify.get('/api/settings', async (request, reply) => {
  try {
    console.log('=== FETCHING SETTINGS ===');
    const rows = db.prepare('SELECT key, value FROM settings').all();
    console.log('Raw settings from database:', rows);
    // Convert array of {key, value} objects to a single object {key: value}
    const settings = rows.reduce((acc, row) => {
      acc[row.key] = row.value;
      return acc;
    }, {});
    console.log('Processed settings object:', settings);
    return settings;
  } catch (error) {
    console.error('Error fetching settings:', error);
    reply.status(500).send({ error: 'Failed to fetch settings' });
  }
});

fastify.post('/api/settings', async (request, reply) => {
  try {
    const { key, value } = request.body;
    console.log('=== SAVING SETTING ===');
    console.log('Key:', key);
    console.log('Value:', value);
    console.log('Value type:', typeof value);
    console.log('Value length:', value ? value.length : 'null/undefined');
    console.log('Request body:', JSON.stringify(request.body));
    
    // Special logging for weather API key
    if (key === 'WEATHER_API_KEY') {
      console.log('=== WEATHER API KEY SPECIFIC LOGGING ===');
      console.log('Weather API Key received:', value);
      console.log('Is empty string?', value === '');
      console.log('Is null?', value === null);
      console.log('Is undefined?', value === undefined);
    }
    
    if (!key) {
      console.error('ERROR: Missing key in request');
      return reply.status(400).send({ error: 'Key is required.' });
    }
    
    if (value === undefined) {
      console.error('ERROR: Missing value in request');
      return reply.status(400).send({ error: 'Value is required.' });
    }
    
    // Check if database is initialized
    if (!db) {
      console.error('ERROR: Database not initialized');
      return reply.status(500).send({ error: 'Database not initialized. Please restart the server.' });
    }
    
    try {
      // Use INSERT OR REPLACE to either insert a new setting or update an existing one
      const stmt = db.prepare('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)');
      const result = stmt.run(key, String(value)); // Ensure value is a string
      console.log('Database insert result:', result);
      
      // Verify the setting was saved
      const verification = db.prepare('SELECT key, value FROM settings WHERE key = ?').get(key);
      console.log('Verification query result:', verification);
      
      // Special verification for weather API key
      if (key === 'WEATHER_API_KEY') {
        console.log('=== WEATHER API KEY VERIFICATION ===');
        console.log('Saved value in DB:', verification ? verification.value : 'NOT FOUND');
        console.log('Value matches input?', verification && verification.value === String(value));
      }
      
      return { success: true, message: `Setting '${key}' saved successfully.` };
    } catch (dbError) {
      console.error(`Database error saving setting '${key}':`, dbError);
      console.error('Database error stack:', dbError.stack);
      return reply.status(500).send({ 
        error: `Failed to save setting '${key}'`,
        details: dbError.message 
      });
    }
  } catch (error) {
    console.error('Unexpected error in /api/settings:', error);
    console.error('Error stack:', error.stack);
    return reply.status(500).send({ 
      error: 'Unexpected error occurred',
      details: error.message 
    });
  }
});

// DEBUG: Specific endpoint to test API key saving
fastify.post('/api/test-api-key', async (request, reply) => {
  const { apiKey } = request.body;
  console.log('=== TESTING API KEY SAVE ===');
  console.log('Received API key:', apiKey);
  console.log('API key type:', typeof apiKey);
  console.log('API key length:', apiKey ? apiKey.length : 'null/undefined');
  
  try {
    // Test direct database insertion
    const stmt = db.prepare('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)');
    const result = stmt.run('WEATHER_API_KEY', apiKey);
    console.log('Direct insert result:', result);
    
    // Verify it was saved
    const verification = db.prepare('SELECT key, value FROM settings WHERE key = ?').get('WEATHER_API_KEY');
    console.log('Verification result:', verification);
    
    return { 
      success: true, 
      message: 'API key test completed',
      saved: verification 
    };
  } catch (error) {
    console.error('Test API key save error:', error);
    return reply.status(500).send({ error: error.message });
  }
});

// NEW: Generic CORS Proxy Endpoint
fastify.get('/api/proxy', async (request, reply) => {
  console.log('=== PROXY REQUEST RECEIVED ===');
  console.log('Query params:', request.query);
  console.log('Headers:', request.headers);
  
  const { targetUrl } = request.query;

  if (!targetUrl) {
    console.log('ERROR: No targetUrl provided');
    return reply.status(400).send({ error: 'targetUrl query parameter is required.' });
  }

  console.log('Target URL requested:', targetUrl);

  let whitelist = [];
  try {
    // Fetch whitelist from DB. It should be a comma-separated string of hostnames.
    const whitelistSetting = db.prepare('SELECT value FROM settings WHERE key = ?').get('PROXY_WHITELIST');
    if (whitelistSetting && whitelistSetting.value) {
      whitelist = whitelistSetting.value.split(',').map(domain => domain.trim());
    }
    console.log('Current whitelist:', whitelist);
  } catch (dbError) {
    console.error('Error fetching proxy whitelist from settings:', dbError);
    whitelist = []; // Default to empty for security
  }

  // For immediate use, we'll ensure the calendar API is always allowed.
  // A more robust solution might involve seeding this in the database on startup.
  if (!whitelist.includes('calapi.inadiutorium.cz')) {
    whitelist.push('calapi.inadiutorium.cz');
    console.log('Added calapi.inadiutorium.cz to whitelist');
  }

  try {
    const target = new URL(targetUrl);
    const targetHostname = target.hostname;
    console.log('Target hostname:', targetHostname);

    if (!whitelist.includes(targetHostname)) {
      console.warn(`Proxy request blocked for non-whitelisted domain: ${targetHostname}`);
      return reply.status(403).send({ error: 'Access to this domain is not allowed through the proxy.' });
    }

    console.log(`Proxying request to whitelisted domain: ${targetUrl}`);
    
    // Configure axios for both HTTP and HTTPS
    const axiosConfig = {
      timeout: 15000, // 15 second timeout
      headers: {
        'User-Agent': 'HomeGlow-Proxy/1.0',
        'Accept': 'application/json, text/plain, */*',
        'Accept-Encoding': 'gzip, deflate'
      },
      maxRedirects: 5,
      validateStatus: function (status) {
        return status < 500; // Resolve only if the status code is less than 500
      }
    };

    // For HTTP requests, ensure we don't have HTTPS-specific configurations
    if (target.protocol === 'http:') {
      console.log('Making HTTP request (not HTTPS)');
      // No special HTTPS agent needed for HTTP
    } else {
      console.log('Making HTTPS request');
      // For HTTPS, we might need to handle self-signed certificates
      process.env.NODE_TLS_REJECT_UNAUTHORIZED = '0'; // Only for development
    }

    console.log('Making axios request with config:', axiosConfig);
    const response = await axios.get(targetUrl, axiosConfig);
    console.log('Axios response received:', response.status, response.statusText);
    console.log('Response data type:', typeof response.data);
    console.log('Response data preview:', JSON.stringify(response.data).substring(0, 200) + '...');

    // Forward the content type and the data from the external API
    if (response.headers['content-type']) {
      reply.header('Content-Type', response.headers['content-type']);
    }
    
    // Add CORS headers
    reply.header('Access-Control-Allow-Origin', '*');
    reply.header('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS');
    reply.header('Access-Control-Allow-Headers', 'Content-Type, Authorization');
    
    console.log('Sending successful response');
    return reply.status(response.status).send(response.data);

  } catch (error) {
    console.error('Error in proxy request:', error.message);
    console.error('Full error details:', {
      message: error.message,
      code: error.code,
      errno: error.errno,
      syscall: error.syscall,
      address: error.address,
      port: error.port,
      config: error.config ? {
        url: error.config.url,
        method: error.config.method,
        timeout: error.config.timeout
      } : 'No config',
      response: error.response ? {
        status: error.response.status,
        statusText: error.response.statusText,
        data: error.response.data
      } : 'No response'
    });
    
    if (error.response) {
      // Forward the error from the target server
      console.log(`Target server responded with ${error.response.status}: ${error.response.statusText}`);
      return reply.status(error.response.status).send({
        error: `Target server error: ${error.response.status} ${error.response.statusText}`,
        details: error.response.data
      });
    } else if (error.code === 'ECONNREFUSED') {
      console.log(`Connection refused to ${error.address}:${error.port}`);
      return reply.status(503).send({ 
        error: 'Unable to connect to the target server. The server may be down or unreachable.',
        details: `Connection refused to ${error.address}:${error.port}`
      });
    } else if (error.code === 'ENOTFOUND') {
      return reply.status(404).send({ error: 'Target URL not found or unreachable.' });
    } else if (error.code === 'ETIMEDOUT') {
      return reply.status(408).send({ error: 'Request to target URL timed out.' });
    } else if (error.code === 'ECONNRESET') {
      return reply.status(503).send({ error: 'Connection was reset by the target server.' });
    }
    
    // Handle other errors (e.g., network, invalid URL)
    return reply.status(500).send({ 
      error: 'Failed to proxy request.',
      details: error.message 
    });
  }
});

// Prize routes
fastify.get('/api/prizes', async (request, reply) => {
  try {
    const rows = db.prepare('SELECT * FROM prizes').all();
    return rows;
  } catch (error) {
    console.error('Error fetching prizes:', error);
    reply.status(500).send({ error: 'Failed to fetch prizes' });
  }
});

fastify.post('/api/prizes', async (request, reply) => {
  const { name, clam_cost } = request.body;
  if (!name || !clam_cost || clam_cost <= 0) {
    return reply.status(400).send({ error: 'Prize name and a positive clam cost are required.' });
  }
  try {
    const stmt = db.prepare('INSERT INTO prizes (name, clam_cost) VALUES (?, ?)');
    const info = stmt.run(name, clam_cost);
    return { id: info.lastInsertRowid };
  } catch (error) {
    console.error('Error adding prize:', error);
    reply.status(500).send({ error: 'Failed to add prize' });
  }
});

fastify.patch('/api/prizes/:id', async (request, reply) => {
  const { id } = request.params;
  const { name, clam_cost } = request.body;
  if (!name || !clam_cost || clam_cost <= 0) {
    return reply.status(400).send({ error: 'Prize name and a positive clam cost are required.' });
  }
  try {
    const stmt = db.prepare('UPDATE prizes SET name = ?, clam_cost = ? WHERE id = ?');
    const info = stmt.run(name, clam_cost, id);
    if (info.changes === 0) {
      return reply.status(404).send({ error: 'Prize not found' });
    }
    return { success: true, message: 'Prize updated successfully' };
  } catch (error) {
    console.error('Error updating prize:', error);
    reply.status(500).send({ error: 'Failed to update prize' });
  }
});

fastify.delete('/api/prizes/:id', async (request, reply) => {
  const { id } = request.params;
  try {
    const stmt = db.prepare('DELETE FROM prizes WHERE id = ?');
    const info = stmt.run(id);
    if (info.changes === 0) {
      return reply.status(404).send({ error: 'Prize not found' });
    }
    return { success: true, message: 'Prize deleted successfully' };
  } catch (error) {
    console.error('Error deleting prize:', error);
    reply.status(500).send({ error: 'Failed to delete prize' });
  }
});

// GET /api/settings/PRIZE_MINIMUM_SHELLS - Get minimum shells setting
fastify.get('/api/settings/PRIZE_MINIMUM_SHELLS', async (request, reply) => {
  try {
    const setting = db.prepare('SELECT value FROM settings WHERE key = ?').get('PRIZE_MINIMUM_SHELLS');
    return { value: setting ? parseInt(setting.value) || 0 : 0 };
  } catch (error) {
    console.error('Error fetching PRIZE_MINIMUM_SHELLS:', error);
    reply.status(500).send({ error: 'Failed to fetch minimum shells setting' });
  }
});

// PUT /api/settings/PRIZE_MINIMUM_SHELLS - Update minimum shells setting (admin only)
fastify.put('/api/settings/PRIZE_MINIMUM_SHELLS', async (request, reply) => {
  if (requireAdminPIN(request, reply)) return;
  
  const { value } = request.body;
  if (value === undefined || value < 0) {
    return reply.status(400).send({ error: 'Minimum shells must be a non-negative number' });
  }
  try {
    db.prepare('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)').run('PRIZE_MINIMUM_SHELLS', String(value));
    return { success: true, message: 'Minimum shells setting updated successfully' };
  } catch (error) {
    console.error('Error updating PRIZE_MINIMUM_SHELLS:', error);
    reply.status(500).send({ error: 'Failed to update minimum shells setting' });
  }
});

// ==================== MEAL PLANNING ENDPOINTS ====================

// Meal Endpoints
// GET /api/meals - Get all meals (with optional date/weekday filtering)
fastify.get('/api/meals', async (request, reply) => {
  try {
    const { date, weekday, week } = request.query;
    let query = 'SELECT * FROM meals WHERE 1=1';
    const params = [];
    
    if (week) {
      // Get meals for a specific week
      const startDate = new Date(week);
      const endDate = new Date(startDate);
      endDate.setDate(endDate.getDate() + 6);
      
      query += ' AND (date_specific = 1 AND assigned_date >= ? AND assigned_date <= ?) OR (date_specific = 0 AND assigned_weekday = ?)';
      params.push(startDate.toISOString().split('T')[0], endDate.toISOString().split('T')[0]);
    } else if (date) {
      query += ' AND (date_specific = 1 AND assigned_date = ?) OR (date_specific = 0 AND assigned_weekday = ?)';
      const dateObj = new Date(date);
      const weekdayName = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'][dateObj.getDay()];
      params.push(date, weekdayName);
    } else if (weekday) {
      query += ' AND date_specific = 0 AND assigned_weekday = ?';
      params.push(weekday);
    }
    
    query += ' ORDER BY assigned_date ASC, assigned_weekday ASC';
    const rows = db.prepare(query).all(...params);
    
    // Parse ingredients JSON
    return rows.map(meal => ({
      ...meal,
      ingredients: JSON.parse(meal.ingredients || '[]')
    }));
  } catch (error) {
    console.error('Error fetching meals:', error);
    reply.status(500).send({ error: 'Failed to fetch meals' });
  }
});

// GET /api/meals/week/:startDate - Get meals for a specific week
fastify.get('/api/meals/week/:startDate', async (request, reply) => {
  try {
    const { startDate } = request.params;
    const start = new Date(startDate);
    const end = new Date(start);
    end.setDate(end.getDate() + 6);
    
    const startStr = start.toISOString().split('T')[0];
    const endStr = end.toISOString().split('T')[0];
    
    // Get date-specific meals for the week
    const dateSpecificMeals = db.prepare(
      'SELECT * FROM meals WHERE date_specific = 1 AND assigned_date >= ? AND assigned_date <= ?'
    ).all(startStr, endStr);
    
    // Get recurring weekday meals
    const weekdayNames = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'];
    const weekdayMeals = db.prepare(
      'SELECT * FROM meals WHERE date_specific = 0 AND assigned_weekday IN (?, ?, ?, ?, ?, ?, ?)'
    ).all(...weekdayNames);
    
    const allMeals = [...dateSpecificMeals, ...weekdayMeals];
    
    return allMeals.map(meal => ({
      ...meal,
      ingredients: JSON.parse(meal.ingredients || '[]')
    }));
  } catch (error) {
    console.error('Error fetching week meals:', error);
    reply.status(500).send({ error: 'Failed to fetch week meals' });
  }
});

// GET /api/meals/:id - Get specific meal
fastify.get('/api/meals/:id', async (request, reply) => {
  try {
    const { id } = request.params;
    const meal = db.prepare('SELECT * FROM meals WHERE id = ?').get(id);
    if (!meal) {
      return reply.status(404).send({ error: 'Meal not found' });
    }
    return {
      ...meal,
      ingredients: JSON.parse(meal.ingredients || '[]')
    };
  } catch (error) {
    console.error('Error fetching meal:', error);
    reply.status(500).send({ error: 'Failed to fetch meal' });
  }
});

// POST /api/meals - Create new meal (admin PIN required)
fastify.post('/api/meals', async (request, reply) => {
  if (requireAdminPIN(request, reply)) return;
  
  const { title, description, recipe, recipe_url, cooktime, preptime, servings, category, ingredients, date_specific, assigned_date, assigned_weekday } = request.body;
  
  if (!title || !ingredients || !Array.isArray(ingredients)) {
    return reply.status(400).send({ error: 'Title and ingredients array are required' });
  }
  
  if (date_specific === 1 && !assigned_date) {
    return reply.status(400).send({ error: 'assigned_date is required for date-specific meals' });
  }
  
  if (date_specific === 0 && !assigned_weekday) {
    return reply.status(400).send({ error: 'assigned_weekday is required for recurring meals' });
  }
  
  try {
    const stmt = db.prepare(`
      INSERT INTO meals (title, description, recipe, recipe_url, cooktime, preptime, servings, category, ingredients, date_specific, assigned_date, assigned_weekday)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    const info = stmt.run(
      title,
      description || null,
      recipe || null,
      recipe_url || null,
      cooktime || null,
      preptime || null,
      servings || null,
      category || null,
      JSON.stringify(ingredients),
      date_specific || 0,
      assigned_date || null,
      assigned_weekday || null
    );
    return { id: info.lastInsertRowid };
  } catch (error) {
    console.error('Error creating meal:', error);
    reply.status(500).send({ error: 'Failed to create meal' });
  }
});

// PUT /api/meals/:id - Update meal (admin PIN required)
fastify.put('/api/meals/:id', async (request, reply) => {
  if (requireAdminPIN(request, reply)) return;
  
  const { id } = request.params;
  const { title, description, recipe, recipe_url, cooktime, preptime, servings, category, ingredients, date_specific, assigned_date, assigned_weekday } = request.body;
  
  if (!title || !ingredients || !Array.isArray(ingredients)) {
    return reply.status(400).send({ error: 'Title and ingredients array are required' });
  }
  
  try {
    const stmt = db.prepare(`
      UPDATE meals 
      SET title = ?, description = ?, recipe = ?, recipe_url = ?, cooktime = ?, preptime = ?, servings = ?, category = ?, ingredients = ?, date_specific = ?, assigned_date = ?, assigned_weekday = ?, updated_at = CURRENT_TIMESTAMP
      WHERE id = ?
    `);
    const info = stmt.run(
      title,
      description || null,
      recipe || null,
      recipe_url || null,
      cooktime || null,
      preptime || null,
      servings || null,
      category || null,
      JSON.stringify(ingredients),
      date_specific || 0,
      assigned_date || null,
      assigned_weekday || null,
      id
    );
    
    if (info.changes === 0) {
      return reply.status(404).send({ error: 'Meal not found' });
    }
    return { success: true };
  } catch (error) {
    console.error('Error updating meal:', error);
    reply.status(500).send({ error: 'Failed to update meal' });
  }
});

// DELETE /api/meals/:id - Delete meal (admin PIN required)
fastify.delete('/api/meals/:id', async (request, reply) => {
  if (requireAdminPIN(request, reply)) return;
  
  const { id } = request.params;
  try {
    const stmt = db.prepare('DELETE FROM meals WHERE id = ?');
    const info = stmt.run(id);
    if (info.changes === 0) {
      return reply.status(404).send({ error: 'Meal not found' });
    }
    return { success: true };
  } catch (error) {
    console.error('Error deleting meal:', error);
    reply.status(500).send({ error: 'Failed to delete meal' });
  }
});

// Grocery List Endpoints
// GET /api/grocery - Get all grocery items (filtered by checked status)
fastify.get('/api/grocery', async (request, reply) => {
  try {
    const { checked } = request.query;
    let query = 'SELECT * FROM grocery_items WHERE 1=1';
    const params = [];
    
    if (checked !== undefined) {
      query += ' AND checked = ?';
      params.push(checked === 'true' ? 1 : 0);
    }
    
    query += ' ORDER BY checked ASC, created_at DESC';
    const rows = db.prepare(query).all(...params);
    
    return rows.map(item => ({
      ...item,
      meal_ids: JSON.parse(item.meal_ids || '[]')
    }));
  } catch (error) {
    console.error('Error fetching grocery items:', error);
    reply.status(500).send({ error: 'Failed to fetch grocery items' });
  }
});

// POST /api/grocery - Add grocery item (any user)
fastify.post('/api/grocery', async (request, reply) => {
  const { name, quantity, unit, user_id, meal_ids } = request.body;
  
  if (!name) {
    return reply.status(400).send({ error: 'Item name is required' });
  }
  
  try {
    const stmt = db.prepare('INSERT INTO grocery_items (name, quantity, unit, user_id, meal_ids) VALUES (?, ?, ?, ?, ?)');
    const info = stmt.run(
      name,
      quantity || null,
      unit || null,
      user_id || null,
      meal_ids ? JSON.stringify(meal_ids) : null
    );
    return { id: info.lastInsertRowid };
  } catch (error) {
    console.error('Error adding grocery item:', error);
    reply.status(500).send({ error: 'Failed to add grocery item' });
  }
});

// PUT /api/grocery/:id - Update grocery item
fastify.put('/api/grocery/:id', async (request, reply) => {
  const { id } = request.params;
  const { name, quantity, unit, meal_ids } = request.body;
  
  try {
    const stmt = db.prepare('UPDATE grocery_items SET name = ?, quantity = ?, unit = ?, meal_ids = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?');
    const info = stmt.run(
      name,
      quantity || null,
      unit || null,
      meal_ids ? JSON.stringify(meal_ids) : null,
      id
    );
    
    if (info.changes === 0) {
      return reply.status(404).send({ error: 'Grocery item not found' });
    }
    return { success: true };
  } catch (error) {
    console.error('Error updating grocery item:', error);
    reply.status(500).send({ error: 'Failed to update grocery item' });
  }
});

// POST /api/grocery/:id/check - Check/uncheck item (user who added or admin)
fastify.post('/api/grocery/:id/check', async (request, reply) => {
  const { id } = request.params;
  const { checked, user_id } = request.body;
  
  try {
    const item = db.prepare('SELECT * FROM grocery_items WHERE id = ?').get(id);
    if (!item) {
      return reply.status(404).send({ error: 'Grocery item not found' });
    }
    
    // Check if user is admin or the user who added the item
    const pin = request.headers['x-admin-pin'];
    const isAdmin = pin && pin === ADMIN_PIN;
    const canCheck = isAdmin || (item.user_id && item.user_id === user_id);
    
    if (!canCheck) {
      return reply.status(403).send({ error: 'Not authorized to check this item' });
    }
    
    const stmt = db.prepare('UPDATE grocery_items SET checked = ?, checked_by = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?');
    stmt.run(checked ? 1 : 0, checked ? user_id : null, id);
    
    return { success: true };
  } catch (error) {
    console.error('Error checking grocery item:', error);
    reply.status(500).send({ error: 'Failed to check grocery item' });
  }
});

// DELETE /api/grocery/:id - Delete grocery item
fastify.delete('/api/grocery/:id', async (request, reply) => {
  const { id } = request.params;
  try {
    const stmt = db.prepare('DELETE FROM grocery_items WHERE id = ?');
    const info = stmt.run(id);
    if (info.changes === 0) {
      return reply.status(404).send({ error: 'Grocery item not found' });
    }
    return { success: true };
  } catch (error) {
    console.error('Error deleting grocery item:', error);
    reply.status(500).send({ error: 'Failed to delete grocery item' });
  }
});

// POST /api/grocery/clear - Clear all checked items (admin PIN required)
fastify.post('/api/grocery/clear', async (request, reply) => {
  if (requireAdminPIN(request, reply)) return;
  
  try {
    const stmt = db.prepare('DELETE FROM grocery_items WHERE checked = 1');
    stmt.run();
    return { success: true };
  } catch (error) {
    console.error('Error clearing checked items:', error);
    reply.status(500).send({ error: 'Failed to clear checked items' });
  }
});

// POST /api/grocery/generate-from-meals - Auto-generate from meals for date range (admin PIN required)
fastify.post('/api/grocery/generate-from-meals', async (request, reply) => {
  if (requireAdminPIN(request, reply)) return;
  
  const { startDate, endDate } = request.body;
  
  if (!startDate || !endDate) {
    return reply.status(400).send({ error: 'startDate and endDate are required' });
  }
  
  try {
    const start = new Date(startDate);
    const end = new Date(endDate);
    
    // Get all meals in the date range
    const dateSpecificMeals = db.prepare(
      'SELECT * FROM meals WHERE date_specific = 1 AND assigned_date >= ? AND assigned_date <= ?'
    ).all(start.toISOString().split('T')[0], end.toISOString().split('T')[0]);
    
    // Get recurring weekday meals
    const weekdayNames = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'];
    const weekdayMeals = db.prepare(
      'SELECT * FROM meals WHERE date_specific = 0 AND assigned_weekday IN (?, ?, ?, ?, ?, ?, ?)'
    ).all(...weekdayNames);
    
    const allMeals = [...dateSpecificMeals, ...weekdayMeals];
    
    // Aggregate ingredients
    const ingredientMap = new Map();
    
    allMeals.forEach(meal => {
      const ingredients = JSON.parse(meal.ingredients || '[]');
      ingredients.forEach(ing => {
        const key = ing.name.toLowerCase().trim();
        if (ingredientMap.has(key)) {
          const existing = ingredientMap.get(key);
          existing.mealIds.push(meal.id);
          // Try to merge quantities if units match
          if (existing.unit === ing.unit) {
            existing.quantity = (parseFloat(existing.quantity) || 0) + (parseFloat(ing.quantity) || 0);
          }
        } else {
          ingredientMap.set(key, {
            name: ing.name,
            quantity: ing.quantity || null,
            unit: ing.unit || null,
            mealIds: [meal.id]
          });
        }
      });
    });
    
    // Create grocery items
    const insertStmt = db.prepare('INSERT INTO grocery_items (name, quantity, unit, meal_ids) VALUES (?, ?, ?, ?)');
    
    for (const [key, ing] of ingredientMap.entries()) {
      insertStmt.run(
        ing.name,
        ing.quantity ? String(ing.quantity) : null,
        ing.unit || null,
        JSON.stringify(ing.mealIds)
      );
    }
    
    return { success: true, itemsCreated: ingredientMap.size };
  } catch (error) {
    console.error('Error generating grocery list:', error);
    reply.status(500).send({ error: 'Failed to generate grocery list' });
  }
});

// Meal Suggestions Endpoints
// GET /api/meal-suggestions - Get all suggestions (newest first)
fastify.get('/api/meal-suggestions', async (request, reply) => {
  try {
    const rows = db.prepare('SELECT * FROM meal_suggestions ORDER BY created_at DESC').all();
    return rows;
  } catch (error) {
    console.error('Error fetching meal suggestions:', error);
    reply.status(500).send({ error: 'Failed to fetch meal suggestions' });
  }
});

// POST /api/meal-suggestions - Add suggestion (any user)
fastify.post('/api/meal-suggestions', async (request, reply) => {
  const { user_id, suggestion_text } = request.body;
  
  if (!user_id || !suggestion_text) {
    return reply.status(400).send({ error: 'user_id and suggestion_text are required' });
  }
  
  try {
    const stmt = db.prepare('INSERT INTO meal_suggestions (user_id, suggestion_text) VALUES (?, ?)');
    const info = stmt.run(user_id, suggestion_text);
    return { id: info.lastInsertRowid };
  } catch (error) {
    console.error('Error adding meal suggestion:', error);
    reply.status(500).send({ error: 'Failed to add meal suggestion' });
  }
});

// DELETE /api/meal-suggestions/:id - Delete suggestion (admin PIN required)
fastify.delete('/api/meal-suggestions/:id', async (request, reply) => {
  if (requireAdminPIN(request, reply)) return;
  
  const { id } = request.params;
  try {
    const stmt = db.prepare('DELETE FROM meal_suggestions WHERE id = ?');
    const info = stmt.run(id);
    if (info.changes === 0) {
      return reply.status(404).send({ error: 'Meal suggestion not found' });
    }
    return { success: true };
  } catch (error) {
    console.error('Error deleting meal suggestion:', error);
    reply.status(500).send({ error: 'Failed to delete meal suggestion' });
  }
});

// POST /api/meal-suggestions/:id/convert - Convert suggestion to meal (admin PIN required)
fastify.post('/api/meal-suggestions/:id/convert', async (request, reply) => {
  if (requireAdminPIN(request, reply)) return;
  
  const { id } = request.params;
  try {
    const suggestion = db.prepare('SELECT * FROM meal_suggestions WHERE id = ?').get(id);
    if (!suggestion) {
      return reply.status(404).send({ error: 'Meal suggestion not found' });
    }
    
    // Return the suggestion so frontend can pre-fill the meal form
    return { 
      success: true, 
      suggestion: suggestion,
      message: 'Suggestion ready to convert to meal'
    };
  } catch (error) {
    console.error('Error converting meal suggestion:', error);
    reply.status(500).send({ error: 'Failed to convert meal suggestion' });
  }
});

// POST /api/prizes/select - Select a prize and deduct shells
fastify.post('/api/prizes/select', async (request, reply) => {
  const { user_id, prize_id } = request.body;
  if (!user_id || !prize_id) {
    return reply.status(400).send({ error: 'User ID and Prize ID are required' });
  }
  try {
    // Get user's current shell balance
    const user = db.prepare('SELECT clam_total FROM users WHERE id = ?').get(user_id);
    if (!user) {
      return reply.status(404).send({ error: 'User not found' });
    }
    
    // Get prize details
    const prize = db.prepare('SELECT * FROM prizes WHERE id = ?').get(prize_id);
    if (!prize) {
      return reply.status(404).send({ error: 'Prize not found' });
    }
    
    // Check if user has enough shells
    if (user.clam_total < prize.clam_cost) {
      return reply.status(400).send({ error: 'Insufficient shells to purchase this prize' });
    }
    
    // Get minimum shells setting
    const minShellsSetting = db.prepare('SELECT value FROM settings WHERE key = ?').get('PRIZE_MINIMUM_SHELLS');
    const minShells = minShellsSetting ? parseInt(minShellsSetting.value) || 0 : 0;
    
    // Check if user meets minimum requirement
    if (user.clam_total < minShells) {
      return reply.status(400).send({ error: `User must have at least ${minShells} shells to spin for prizes` });
    }
    
    // Deduct shells
    const updateStmt = db.prepare('UPDATE users SET clam_total = clam_total - ? WHERE id = ?');
    updateStmt.run(prize.clam_cost, user_id);
    
    return { 
      success: true, 
      prize: prize,
      remaining_shells: user.clam_total - prize.clam_cost
    };
  } catch (error) {
    console.error('Error selecting prize:', error);
    reply.status(500).send({ error: 'Failed to select prize' });
  }
});

// Calendar sources routes
fastify.get('/api/calendar-sources', async (request, reply) => {
  try {
    const rows = db.prepare('SELECT id, name, type, url, username, color, enabled, sort_order, created_at FROM calendar_sources ORDER BY sort_order, id').all();
    return rows;
  } catch (error) {
    console.error('Error fetching calendar sources:', error);
    reply.status(500).send({ error: 'Failed to fetch calendar sources' });
  }
});

fastify.post('/api/calendar-sources', async (request, reply) => {
  const { name, type, url, username, password, color } = request.body;
  if (!name || !url) {
    return reply.status(400).send({ error: 'Name and URL are required.' });
  }
  // Only CalDAV is supported now
  if (type && type !== 'CalDAV') {
    return reply.status(400).send({ error: 'Only CalDAV calendars are supported.' });
  }
  try {
    const encryptedPassword = password ? encryptPassword(password) : null;
    const maxOrder = db.prepare('SELECT MAX(sort_order) as max FROM calendar_sources').get();
    const nextOrder = (maxOrder.max || 0) + 1;

    const stmt = db.prepare(`
      INSERT INTO calendar_sources (name, type, url, username, password, color, enabled, sort_order)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `);
    const info = stmt.run(name, 'CalDAV', url, username || null, encryptedPassword, color || '#6e44ff', 1, nextOrder);
    return { id: info.lastInsertRowid, success: true };
  } catch (error) {
    console.error('Error adding calendar source:', error);
    reply.status(500).send({ error: 'Failed to add calendar source' });
  }
});

fastify.patch('/api/calendar-sources/:id', async (request, reply) => {
  const { id } = request.params;
  const { name, type, url, username, password, color, enabled } = request.body;

  try {
    const existing = db.prepare('SELECT * FROM calendar_sources WHERE id = ?').get(id);
    if (!existing) {
      return reply.status(404).send({ error: 'Calendar source not found' });
    }

    const updateFields = [];
    const updateValues = [];

    if (name !== undefined) { updateFields.push('name = ?'); updateValues.push(name); }
    if (type !== undefined) {
      if (type !== 'CalDAV') {
        return reply.status(400).send({ error: 'Only CalDAV calendars are supported.' });
      }
      updateFields.push('type = ?');
      updateValues.push('CalDAV');
    }
    if (url !== undefined) { updateFields.push('url = ?'); updateValues.push(url); }
    if (username !== undefined) { updateFields.push('username = ?'); updateValues.push(username || null); }
    if (password !== undefined && password !== '') {
      const encryptedPassword = encryptPassword(password);
      updateFields.push('password = ?');
      updateValues.push(encryptedPassword);
    }
    if (color !== undefined) { updateFields.push('color = ?'); updateValues.push(color); }
    if (enabled !== undefined) { updateFields.push('enabled = ?'); updateValues.push(enabled ? 1 : 0); }

    if (updateFields.length === 0) {
      return reply.status(400).send({ error: 'No fields to update' });
    }

    updateValues.push(id);
    const stmt = db.prepare(`UPDATE calendar_sources SET ${updateFields.join(', ')} WHERE id = ?`);
    const info = stmt.run(...updateValues);

    if (info.changes === 0) {
      return reply.status(404).send({ error: 'Calendar source not found' });
    }
    return { success: true, message: 'Calendar source updated successfully' };
  } catch (error) {
    console.error('Error updating calendar source:', error);
    reply.status(500).send({ error: 'Failed to update calendar source' });
  }
});

fastify.delete('/api/calendar-sources/:id', async (request, reply) => {
  const { id } = request.params;
  try {
    const stmt = db.prepare('DELETE FROM calendar_sources WHERE id = ?');
    const info = stmt.run(id);
    if (info.changes === 0) {
      return reply.status(404).send({ error: 'Calendar source not found' });
    }
    return { success: true, message: 'Calendar source deleted successfully' };
  } catch (error) {
    console.error('Error deleting calendar source:', error);
    reply.status(500).send({ error: 'Failed to delete calendar source' });
  }
});

fastify.post('/api/calendar-sources/:id/test', async (request, reply) => {
  const { id } = request.params;
  try {
    const source = db.prepare('SELECT * FROM calendar_sources WHERE id = ?').get(id);
    if (!source) {
      return reply.status(404).send({ error: 'Calendar source not found' });
    }

    // Only CalDAV is supported
    if (source.type !== 'CalDAV') {
      return reply.status(400).send({ error: 'Only CalDAV calendars are supported.' });
    }

    const decryptedPassword = decryptPassword(source.password);
    const authHeader = 'Basic ' + Buffer.from(`${source.username}:${decryptedPassword}`).toString('base64');
    const response = await axios.get(source.url, {
      headers: { 'Authorization': authHeader },
      timeout: 10000
    });
    return { success: true, message: 'CalDAV connection successful' };
  } catch (error) {
    console.error('Error testing calendar source:', error);
    return reply.status(400).send({
      success: false,
      error: 'Failed to connect to calendar source',
      details: error.message
    });
  }
});


// Helper function to get timezone setting
function getTimezone() {
  try {
    const setting = db.prepare('SELECT value FROM settings WHERE key = ?').get('TIMEZONE');
    if (!setting || !setting.value) {
      // Default to system timezone or America/New_York
      return Intl.DateTimeFormat().resolvedOptions().timeZone || 'America/New_York';
    }
    return setting.value;
  } catch (error) {
    console.error('Error getting timezone:', error);
    return 'America/New_York';
  }
}

// Helper function to get the date components (year, month, day) of a Date in a specific timezone
function getDateComponentsInTimezone(date, timezone) {
  const formatter = new Intl.DateTimeFormat('en-CA', {
    timeZone: timezone,
    year: 'numeric',
    month: '2-digit',
    day: '2-digit'
  });
  const parts = formatter.formatToParts(date);
  return {
    year: parseInt(parts.find(p => p.type === 'year').value),
    month: parseInt(parts.find(p => p.type === 'month').value),
    day: parseInt(parts.find(p => p.type === 'day').value)
  };
}

// Helper function to check if a date falls on a specific day in a timezone
function isDateOnDay(date, dateString, timezone) {
  const [year, month, day] = dateString.split('-').map(Number);
  const dateComponents = getDateComponentsInTimezone(date, timezone);
  return dateComponents.year === year && 
         dateComponents.month === month && 
         dateComponents.day === day;
}

// Helper function to get default calendar
function getDefaultCalendar() {
  try {
    const setting = db.prepare('SELECT value FROM settings WHERE key = ?').get('DEFAULT_CALENDAR_ID');
    if (!setting || !setting.value) {
      return null;
    }
    const calendarId = parseInt(setting.value);
    const calendar = db.prepare('SELECT * FROM calendar_sources WHERE id = ? AND enabled = 1 AND type = ?').get(calendarId, 'CalDAV');
    return calendar || null;
  } catch (error) {
    console.error('Error getting default calendar:', error);
    return null;
  }
}

// Endpoint to set default calendar
fastify.put('/api/settings/DEFAULT_CALENDAR_ID', async (request, reply) => {
  const { value } = request.body;
  if (!value) {
    return reply.status(400).send({ error: 'Calendar ID is required.' });
  }
  try {
    const calendarId = parseInt(value);
    const calendar = db.prepare('SELECT * FROM calendar_sources WHERE id = ? AND enabled = 1 AND type = ?').get(calendarId, 'CalDAV');
    if (!calendar) {
      return reply.status(404).send({ error: 'Calendar not found or not enabled.' });
    }
    db.prepare('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)').run('DEFAULT_CALENDAR_ID', value);
    return { success: true, message: 'Default calendar set successfully' };
  } catch (error) {
    console.error('Error setting default calendar:', error);
    reply.status(500).send({ error: 'Failed to set default calendar' });
  }
});

// Enhanced endpoint to fetch and parse calendar events from CalDAV sources only
fastify.get('/api/calendar-events', async (request, reply) => {
  try {
    const sources = db.prepare('SELECT * FROM calendar_sources WHERE enabled = 1 AND type = ? ORDER BY sort_order, id').all('CalDAV');

    if (sources.length === 0) {
      return [];
    }

    const fetchPromises = sources.map(async (source) => {
      try {
        const decryptedPassword = decryptPassword(source.password);
        const authHeader = 'Basic ' + Buffer.from(`${source.username}:${decryptedPassword}`).toString('base64');

        // Use REPORT request for proper CalDAV querying (like todos endpoint)
        const reportBody = `<?xml version="1.0" encoding="utf-8" ?>
<c:calendar-query xmlns:d="DAV:" xmlns:c="urn:ietf:params:xml:ns:caldav">
  <d:prop>
    <d:getetag />
    <c:calendar-data />
  </d:prop>
  <c:filter>
    <c:comp-filter name="VCALENDAR">
      <c:comp-filter name="VEVENT" />
    </c:comp-filter>
  </c:filter>
</c:calendar-query>`;

        let response;
        let icsData;
        
        try {
          // Try REPORT request first (proper CalDAV method)
          response = await axios.request({
            method: 'REPORT',
            url: source.url,
            headers: {
              'Authorization': authHeader,
              'Content-Type': 'application/xml; charset=utf-8',
              'Depth': '1'
            },
            data: reportBody,
            timeout: 15000
          });
        } catch (reportError) {
          // Fallback to GET if REPORT fails
          console.log(`REPORT request failed for ${source.name}, falling back to GET:`, reportError.message);
          response = await axios.get(source.url, {
            headers: { 
              'Authorization': authHeader,
              'Accept': 'text/calendar',
              'Depth': '1'
            },
            timeout: 15000
          });
        }

        // Handle empty responses gracefully
        if (!response.data || response.data.trim() === '') {
          console.log(`Empty response from calendar URL: ${source.url}`);
          return [];
        }

        icsData = response.data;
        
        // If response is XML (from REPORT), extract calendar-data
        // Handle both c: and cal: namespace prefixes (Davis uses cal:)
        let vevents = [];
        
        if (typeof icsData === 'string' && (icsData.includes('<?xml') || icsData.includes('<d:multistatus'))) {
          console.log(`Detected XML response for ${source.name}, extracting calendar-data...`);
          
          // Try to extract calendar-data with CDATA first (handles both c: and cal: prefixes)
          let calendarDataMatches = icsData.match(/<(?:c|cal):calendar-data[^>]*><!\[CDATA\[([\s\S]*?)\]\]><\/(?:c|cal):calendar-data>/g);
          if (calendarDataMatches && calendarDataMatches.length > 0) {
            console.log(`Found ${calendarDataMatches.length} calendar-data sections with CDATA for ${source.name}`);
          } else {
            // Try without CDATA - need to handle multiline content
            calendarDataMatches = icsData.match(/<(?:c|cal):calendar-data[^>]*>([\s\S]*?)<\/(?:c|cal):calendar-data>/g);
            if (calendarDataMatches && calendarDataMatches.length > 0) {
              console.log(`Found ${calendarDataMatches.length} calendar-data sections without CDATA for ${source.name}`);
            } else {
              console.log(`No calendar-data found in XML response for ${source.name}`);
              console.log('Response preview:', icsData.substring(0, 1000));
              return [];
            }
          }
          
          // If we extracted multiple calendar items, parse each separately
          // Each calendar-data section is a complete VCALENDAR
          if (calendarDataMatches && calendarDataMatches.length > 1) {
            console.log(`Parsing ${calendarDataMatches.length} separate calendar items for ${source.name}...`);
            const allVevents = [];
            
            for (const match of calendarDataMatches) {
              let itemData;
              if (match.includes('CDATA')) {
                const cdataMatch = match.match(/<!\[CDATA\[([\s\S]*?)\]\]>/);
                itemData = cdataMatch ? cdataMatch[1] : '';
              } else {
                const contentMatch = match.match(/>([\s\S]*?)<\/(?:c|cal):calendar-data>/);
                if (contentMatch) {
                  itemData = contentMatch[1].replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&').replace(/&quot;/g, '"');
                }
              }
              
              if (itemData && itemData.trim()) {
                try {
                  const itemJcal = ICAL.parse(itemData.trim());
                  const itemComp = new ICAL.Component(itemJcal);
                  const itemVevents = itemComp.getAllSubcomponents('vevent');
                  allVevents.push(...itemVevents);
                } catch (parseError) {
                  console.warn(`Failed to parse individual calendar item for ${source.name}:`, parseError.message);
                }
              }
            }
            
            console.log(`Found ${allVevents.length} VEVENT components across all calendar items for ${source.name}`);
            vevents = allVevents;
          } else if (calendarDataMatches && calendarDataMatches.length === 1) {
            // Single calendar item
            let itemData;
            if (calendarDataMatches[0].includes('CDATA')) {
              const cdataMatch = calendarDataMatches[0].match(/<!\[CDATA\[([\s\S]*?)\]\]>/);
              itemData = cdataMatch ? cdataMatch[1] : '';
            } else {
              const contentMatch = calendarDataMatches[0].match(/>([\s\S]*?)<\/(?:c|cal):calendar-data>/);
              if (contentMatch) {
                itemData = contentMatch[1].replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&').replace(/&quot;/g, '"');
              }
            }
            
            if (itemData && itemData.trim()) {
              try {
                const itemJcal = ICAL.parse(itemData.trim());
                const itemComp = new ICAL.Component(itemJcal);
                vevents = itemComp.getAllSubcomponents('vevent');
              } catch (parseError) {
                console.warn(`Failed to parse calendar data for ${source.name}:`, parseError.message);
                return [];
              }
            }
          }
        } else {
          // Direct iCal response (from GET fallback)
          try {
            const jcalData = ICAL.parse(icsData);
            const comp = new ICAL.Component(jcalData);
            vevents = comp.getAllSubcomponents('vevent');
          } catch (parseError) {
            console.warn(`Failed to parse iCal data for ${source.name}:`, parseError.message);
            return [];
          }
        }

        // Map vevents to event objects
        // Note: getAllSubcomponents returns ICAL.Component instances
        const events = vevents.map(vevent => {
          try {
            const event = new ICAL.Event(vevent);
            return {
              id: event.uid || Math.random().toString(),
              title: event.summary || 'Untitled Event',
              start: event.startDate.toJSDate(),
              end: event.endDate.toJSDate(),
              description: event.description || '',
              location: event.location || '',
              source_id: source.id,
              source_name: source.name,
              source_color: source.color
            };
          } catch (eventError) {
            console.warn(`Failed to create event from vevent for ${source.name}:`, eventError.message);
            return null;
          }
        }).filter(event => event !== null);

        console.log(`Fetched ${events.length} events from ${source.name}`);
        return events;
      } catch (error) {
        console.error(`Error fetching calendar from source ${source.name}:`, error.message);
        return [];
      }
    });

    const results = await Promise.all(fetchPromises);
    const allEvents = results.flat();

    return allEvents.sort((a, b) => new Date(a.start) - new Date(b.start));
  } catch (error) {
    console.error('Error fetching calendar events:', error);
    reply.status(500).send({ error: 'Failed to fetch calendar events.' });
  }
});

// Helper function to get CalDAV auth header
function getCalDAVAuthHeader(source) {
  const decryptedPassword = decryptPassword(source.password);
  return 'Basic ' + Buffer.from(`${source.username}:${decryptedPassword}`).toString('base64');
}

// Helper function to generate UID
function generateUID() {
  return `${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
}

// GET /api/caldav/todos - Fetch todos from default calendar
fastify.get('/api/caldav/todos', async (request, reply) => {
  try {
    const defaultCalendar = getDefaultCalendar();
    if (!defaultCalendar) {
      return reply.status(400).send({ error: 'No default calendar set. Please set a default calendar in Admin Panel.' });
    }

    const { date } = request.query;
    const authHeader = getCalDAVAuthHeader(defaultCalendar);

    // For CalDAV, use REPORT request to query for VTODO components
    // This is the proper way to fetch all todos from a calendar collection
    const reportBody = `<?xml version="1.0" encoding="utf-8" ?>
<c:calendar-query xmlns:d="DAV:" xmlns:c="urn:ietf:params:xml:ns:caldav">
  <d:prop>
    <d:getetag />
    <c:calendar-data />
  </d:prop>
  <c:filter>
    <c:comp-filter name="VCALENDAR">
      <c:comp-filter name="VTODO" />
    </c:comp-filter>
  </c:filter>
</c:calendar-query>`;

    let response;
    try {
      // Try REPORT request first (proper CalDAV method)
      response = await axios.request({
        method: 'REPORT',
        url: defaultCalendar.url,
        headers: {
          'Authorization': authHeader,
          'Content-Type': 'application/xml; charset=utf-8',
          'Depth': '1'
        },
        data: reportBody,
        timeout: 15000
      });
    } catch (reportError) {
      // If REPORT fails, fall back to GET (some servers support it)
      console.log('REPORT request failed, trying GET:', reportError.message);
      response = await axios.get(defaultCalendar.url, {
        headers: { 
          'Authorization': authHeader,
          'Accept': 'text/calendar',
          'Depth': '1'
        },
        timeout: 15000
      });
    }

    // Handle empty responses gracefully
    if (!response.data || response.data.trim() === '') {
      console.log('Empty response from calendar URL:', defaultCalendar.url);
      return [];
    }

    let icsData = response.data;
    
    // If response is XML (from REPORT), extract calendar-data
    // Handle both c: and cal: namespace prefixes (Davis uses cal:)
    if (typeof icsData === 'string' && (icsData.includes('<?xml') || icsData.includes('<d:multistatus'))) {
      console.log('Detected XML response, extracting calendar-data...');
      
      // Try to extract calendar-data with CDATA first (handles both c: and cal: prefixes)
      let calendarDataMatches = icsData.match(/<(?:c|cal):calendar-data[^>]*><!\[CDATA\[([\s\S]*?)\]\]><\/(?:c|cal):calendar-data>/g);
      if (calendarDataMatches && calendarDataMatches.length > 0) {
        console.log(`Found ${calendarDataMatches.length} calendar-data sections with CDATA`);
        // Combine all calendar data into one iCal string
        icsData = calendarDataMatches.map(match => {
          const cdataMatch = match.match(/<!\[CDATA\[([\s\S]*?)\]\]>/);
          return cdataMatch ? cdataMatch[1] : '';
        }).join('\n');
      } else {
        // Try without CDATA - need to handle multiline content
        // Use a more flexible regex that handles content across multiple lines
        calendarDataMatches = icsData.match(/<(?:c|cal):calendar-data[^>]*>([\s\S]*?)<\/(?:c|cal):calendar-data>/g);
        if (calendarDataMatches && calendarDataMatches.length > 0) {
          console.log(`Found ${calendarDataMatches.length} calendar-data sections without CDATA`);
          icsData = calendarDataMatches.map(match => {
            // Extract content between > and </
            const contentMatch = match.match(/>([\s\S]*?)<\/(?:c|cal):calendar-data>/);
            if (contentMatch) {
              let content = contentMatch[1];
              // Unescape XML entities
              content = content.replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&').replace(/&quot;/g, '"');
              return content;
            }
            return '';
          }).join('\n');
        } else {
          console.log('No calendar-data found in XML response');
          console.log('Response preview:', icsData.substring(0, 1000));
          return [];
        }
      }
      
      // If we extracted multiple calendar items, parse each separately
      // Each calendar-data section is a complete VCALENDAR
      if (calendarDataMatches && calendarDataMatches.length > 1) {
        console.log(`Parsing ${calendarDataMatches.length} separate calendar items...`);
        const allVtodos = [];
        
        for (const match of calendarDataMatches) {
          let itemData;
          if (match.includes('CDATA')) {
            const cdataMatch = match.match(/<!\[CDATA\[([\s\S]*?)\]\]>/);
            itemData = cdataMatch ? cdataMatch[1] : '';
          } else {
            const contentMatch = match.match(/>([\s\S]*?)<\/(?:c|cal):calendar-data>/);
            if (contentMatch) {
              itemData = contentMatch[1].replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&').replace(/&quot;/g, '"');
            }
          }
          
          if (itemData && itemData.trim()) {
            try {
              const itemJcal = ICAL.parse(itemData.trim());
              const itemComp = new ICAL.Component(itemJcal);
              const itemVtodos = itemComp.getAllSubcomponents('vtodo');
              allVtodos.push(...itemVtodos);
            } catch (parseError) {
              console.warn('Failed to parse individual calendar item:', parseError.message);
            }
          }
        }
        
        console.log(`Found ${allVtodos.length} VTODO components across all calendar items`);
        // Use the collected vtodos and continue with mapping below
        const vtodos = allVtodos;
        
        // Map vtodos to todo objects (shared logic)
        // Note: getAllSubcomponents returns ICAL.Component instances, don't wrap them
        let todos = vtodos.map(vtodo => {
          const summary = vtodo.getFirstPropertyValue('summary') || '';
          const description = vtodo.getFirstPropertyValue('description') || '';
          const due = vtodo.getFirstPropertyValue('due');
          const completed = vtodo.getFirstPropertyValue('completed');
          const status = vtodo.getFirstPropertyValue('status');
          const uid = vtodo.getFirstPropertyValue('uid') || generateUID();
          
          // Check for assigned user in description or custom property
          let assignedUser = null;
          const descMatch = description.match(/ASSIGNED_USER:(\d+)/);
          if (descMatch) {
            assignedUser = parseInt(descMatch[1]);
          }

          const todoData = {
            uid: uid,
            summary: summary,
            description: description.replace(/ASSIGNED_USER:\d+\s*/, ''), // Remove assignment marker
            due: due ? new Date(due.toJSDate()) : null,
            completed: completed ? new Date(completed.toJSDate()) : null,
            status: status || 'NEEDS-ACTION',
            assigned_user: assignedUser,
            source_id: defaultCalendar.id,
            source_name: defaultCalendar.name,
            source_color: defaultCalendar.color
          };

          // Filter by date if provided
          if (date) {
            const timezone = getTimezone();
            
            if (todoData.due) {
              const dueDate = new Date(todoData.due);
              // Check if the due date falls on the filter date in the configured timezone
              if (isDateOnDay(dueDate, date, timezone)) {
                return todoData;
              }
            } else {
              // If no due date, include if not completed
              if (todoData.status !== 'COMPLETED') {
                return todoData;
              }
            }
            return null;
          }

          return todoData;
        }).filter(todo => todo !== null);

        // Filter out completed todos
        todos = todos.filter(todo => todo.status !== 'COMPLETED');

        return todos.sort((a, b) => {
          if (a.due && b.due) return new Date(a.due) - new Date(b.due);
          if (a.due) return -1;
          if (b.due) return 1;
          return 0;
        });
      }
      
      // Clean up the extracted iCal data (single calendar item)
      icsData = icsData.trim();
      console.log(`Extracted iCal data (${icsData.length} chars), first 200 chars:`, icsData.substring(0, 200));
    }
    
    console.log(`Fetched calendar data (${icsData.length} chars), looking for VTODO components...`);
    
    let jcalData;
    try {
      jcalData = ICAL.parse(icsData);
    } catch (parseError) {
      // If parsing fails, return empty array (calendar might be empty or invalid)
      console.warn('Failed to parse calendar data, returning empty todos:', parseError.message);
      console.log('Response data preview:', icsData.substring(0, 500));
      return [];
    }
    
    const comp = new ICAL.Component(jcalData);
    const vtodos = comp.getAllSubcomponents('vtodo');
    console.log(`Found ${vtodos.length} VTODO components in calendar`);

    // Note: getAllSubcomponents returns ICAL.Component instances, don't wrap them
    let todos = vtodos.map(vtodo => {
      const summary = vtodo.getFirstPropertyValue('summary') || '';
      const description = vtodo.getFirstPropertyValue('description') || '';
      const due = vtodo.getFirstPropertyValue('due');
      const completed = vtodo.getFirstPropertyValue('completed');
      const status = vtodo.getFirstPropertyValue('status');
      const uid = vtodo.getFirstPropertyValue('uid') || generateUID();
      
      // Check for assigned user in description or custom property
      let assignedUser = null;
      const descMatch = description.match(/ASSIGNED_USER:(\d+)/);
      if (descMatch) {
        assignedUser = parseInt(descMatch[1]);
      }

      const todoData = {
        uid: uid,
        summary: summary,
        description: description.replace(/ASSIGNED_USER:\d+\s*/, ''), // Remove assignment marker
        due: due ? new Date(due.toJSDate()) : null,
        completed: completed ? new Date(completed.toJSDate()) : null,
        status: status || 'NEEDS-ACTION',
        assigned_user: assignedUser,
        source_id: defaultCalendar.id,
        source_name: defaultCalendar.name,
        source_color: defaultCalendar.color
      };

      // Filter by date if provided
      if (date) {
        const timezone = getTimezone();
        
        if (todoData.due) {
          const dueDate = new Date(todoData.due);
          // Check if the due date falls on the filter date in the configured timezone
          if (isDateOnDay(dueDate, date, timezone)) {
            return todoData;
          }
        } else {
          // If no due date, include if not completed
          if (todoData.status !== 'COMPLETED') {
            return todoData;
          }
        }
        return null;
      }

      return todoData;
    }).filter(todo => todo !== null);

    // Filter out completed todos
    todos = todos.filter(todo => todo.status !== 'COMPLETED');

    return todos.sort((a, b) => {
      if (a.due && b.due) return new Date(a.due) - new Date(b.due);
      if (a.due) return -1;
      if (b.due) return 1;
      return 0;
    });
  } catch (error) {
    console.error('Error fetching todos:', error);
    reply.status(500).send({ error: 'Failed to fetch todos.' });
  }
});

// POST /api/caldav/todos - Create new todo in default calendar
fastify.post('/api/caldav/todos', async (request, reply) => {
  try {
    const defaultCalendar = getDefaultCalendar();
    if (!defaultCalendar) {
      return reply.status(400).send({ error: 'No default calendar set. Please set a default calendar in Admin Panel.' });
    }

    const { summary, description, due, assigned_user } = request.body;
    if (!summary) {
      return reply.status(400).send({ error: 'Summary is required.' });
    }

    const uid = generateUID();
    const now = new Date();
    
    // Build description with assigned user if provided
    let todoDescription = description || '';
    if (assigned_user) {
      todoDescription = `ASSIGNED_USER:${assigned_user} ${todoDescription}`.trim();
    }

    // Create VTODO component using ICAL.js
    const vtodo = new ICAL.Component('vtodo');
    vtodo.addPropertyWithValue('uid', uid);
    vtodo.addPropertyWithValue('dtstamp', ICAL.Time.fromJSDate(now));
    vtodo.addPropertyWithValue('created', ICAL.Time.fromJSDate(now));
    vtodo.addPropertyWithValue('summary', summary);
    if (todoDescription) {
      vtodo.addPropertyWithValue('description', todoDescription);
    }
    if (due) {
      const dueTime = ICAL.Time.fromJSDate(new Date(due));
      vtodo.addPropertyWithValue('due', dueTime);
    }
    vtodo.addPropertyWithValue('status', 'NEEDS-ACTION');

    // Create calendar component with VTODO
    const calComponent = new ICAL.Component('vcalendar');
    calComponent.addPropertyWithValue('version', '2.0');
    calComponent.addPropertyWithValue('prodid', '-//MindPalace//NONSGML v1.0//EN');
    calComponent.addSubcomponent(vtodo);
    const icsString = calComponent.toString();

    // PUT to CalDAV server
    // For Davis/SabreDAV, the calendar URL should be the full calendar collection path
    // e.g., https://dav.example.com/dav/calendars/username/calendarname/
    const authHeader = getCalDAVAuthHeader(defaultCalendar);
    const calendarUrl = defaultCalendar.url.endsWith('/') ? defaultCalendar.url : `${defaultCalendar.url}/`;
    const todoUrl = `${calendarUrl}${uid}.ics`;
    
    console.log(`Creating todo at: ${todoUrl}`);
    
    await axios.put(todoUrl, icsString, {
      headers: {
        'Authorization': authHeader,
        'Content-Type': 'text/calendar'
      },
      timeout: 15000
    });

    return { success: true, uid: uid, message: 'Todo created successfully' };
  } catch (error) {
    console.error('Error creating todo:', error);
    if (error.response) {
      const status = error.response.status;
      const errorData = error.response.data;
      let errorMessage = `CalDAV server returned ${status}: `;
      
      // Try to extract error message from XML response
      if (typeof errorData === 'string' && errorData.includes('<s:message>')) {
        const match = errorData.match(/<s:message>(.*?)<\/s:message>/);
        if (match) {
          errorMessage += match[1];
        } else {
          errorMessage += errorData.substring(0, 200);
        }
      } else {
        errorMessage += error.response.statusText || 'Unknown error';
      }
      
      return reply.status(status).send({ 
        error: errorMessage,
        details: `Failed to create todo at ${error.config?.url || 'unknown URL'}. Please check your calendar URL and permissions.`
      });
    }
    reply.status(500).send({ error: 'Failed to create todo.', details: error.message });
  }
});

// PUT /api/caldav/todos/:uid - Update todo
fastify.put('/api/caldav/todos/:uid', async (request, reply) => {
  try {
    const { uid } = request.params;
    const defaultCalendar = getDefaultCalendar();
    if (!defaultCalendar) {
      return reply.status(400).send({ error: 'No default calendar set.' });
    }

    const { summary, description, due, status, assigned_user } = request.body;
    const authHeader = getCalDAVAuthHeader(defaultCalendar);

    // Fetch existing todo
    const calendarUrl = defaultCalendar.url.endsWith('/') ? defaultCalendar.url : `${defaultCalendar.url}/`;
    const todoUrl = `${calendarUrl}${uid}.ics`;
    let existingTodo;
    try {
      const response = await axios.get(todoUrl, {
        headers: { 'Authorization': authHeader },
        timeout: 15000
      });
      const jcalData = ICAL.parse(response.data);
      existingTodo = new ICAL.Component(jcalData);
    } catch (error) {
      return reply.status(404).send({ error: 'Todo not found.' });
    }

    // Update properties
    const vtodo = existingTodo.getFirstSubcomponent('vtodo');
    if (summary !== undefined) vtodo.updatePropertyWithValue('summary', summary);
    if (description !== undefined) {
      let desc = description;
      if (assigned_user) {
        desc = `ASSIGNED_USER:${assigned_user} ${desc}`.trim();
      }
      vtodo.updatePropertyWithValue('description', desc);
    }
    if (due !== undefined) {
      if (due) {
        const dueTime = ICAL.Time.fromJSDate(new Date(due));
        vtodo.updatePropertyWithValue('due', dueTime);
      } else {
        vtodo.removeProperty('due');
      }
    }
    if (status !== undefined) {
      vtodo.updatePropertyWithValue('status', status);
      if (status === 'COMPLETED') {
        vtodo.updatePropertyWithValue('completed', ICAL.Time.fromJSDate(new Date()));
      }
    }

    // PUT updated todo
    await axios.put(todoUrl, existingTodo.toString(), {
      headers: {
        'Authorization': authHeader,
        'Content-Type': 'text/calendar'
      },
      timeout: 15000
    });

    return { success: true, message: 'Todo updated successfully' };
  } catch (error) {
    console.error('Error updating todo:', error);
    reply.status(500).send({ error: 'Failed to update todo.' });
  }
});

// DELETE /api/caldav/todos/:uid - Delete todo
fastify.delete('/api/caldav/todos/:uid', async (request, reply) => {
  try {
    const { uid } = request.params;
    const defaultCalendar = getDefaultCalendar();
    if (!defaultCalendar) {
      return reply.status(400).send({ error: 'No default calendar set.' });
    }

    const authHeader = getCalDAVAuthHeader(defaultCalendar);
    const calendarUrl = defaultCalendar.url.endsWith('/') ? defaultCalendar.url : `${defaultCalendar.url}/`;
    const todoUrl = `${calendarUrl}${uid}.ics`;

    await axios.delete(todoUrl, {
      headers: { 'Authorization': authHeader },
      timeout: 15000
    });

    return { success: true, message: 'Todo deleted successfully' };
  } catch (error) {
    console.error('Error deleting todo:', error);
    if (error.response && error.response.status === 404) {
      return reply.status(404).send({ error: 'Todo not found.' });
    }
    reply.status(500).send({ error: 'Failed to delete todo.' });
  }
});

// GET /api/caldav/notes - Fetch notes from default calendar
fastify.get('/api/caldav/notes', async (request, reply) => {
  try {
    const defaultCalendar = getDefaultCalendar();
    if (!defaultCalendar) {
      return reply.status(400).send({ error: 'No default calendar set. Please set a default calendar in Admin Panel.' });
    }

    const authHeader = getCalDAVAuthHeader(defaultCalendar);

    // For CalDAV, use REPORT request to query for VJOURNAL components
    const reportBody = `<?xml version="1.0" encoding="utf-8" ?>
<c:calendar-query xmlns:d="DAV:" xmlns:c="urn:ietf:params:xml:ns:caldav">
  <d:prop>
    <d:getetag />
    <c:calendar-data />
  </d:prop>
  <c:filter>
    <c:comp-filter name="VCALENDAR">
      <c:comp-filter name="VJOURNAL" />
    </c:comp-filter>
  </c:filter>
</c:calendar-query>`;

    let response;
    try {
      // Try REPORT request first (proper CalDAV method)
      response = await axios.request({
        method: 'REPORT',
        url: defaultCalendar.url,
        headers: {
          'Authorization': authHeader,
          'Content-Type': 'application/xml; charset=utf-8',
          'Depth': '1'
        },
        data: reportBody,
        timeout: 15000
      });
    } catch (reportError) {
      // If REPORT fails, fall back to GET (some servers support it)
      console.log('REPORT request failed, trying GET:', reportError.message);
      response = await axios.get(defaultCalendar.url, {
        headers: { 
          'Authorization': authHeader,
          'Accept': 'text/calendar',
          'Depth': '1'
        },
        timeout: 15000
      });
    }

    // Handle empty responses gracefully
    if (!response.data || response.data.trim() === '') {
      console.log('Empty response from calendar URL:', defaultCalendar.url);
      return [];
    }

    let icsData = response.data;
    
    // If response is XML (from REPORT), extract calendar-data
    // Handle both c: and cal: namespace prefixes (Davis uses cal:)
    if (typeof icsData === 'string' && (icsData.includes('<?xml') || icsData.includes('<d:multistatus'))) {
      console.log('Detected XML response, extracting calendar-data...');
      
      // Try to extract calendar-data with CDATA first (handles both c: and cal: prefixes)
      let calendarDataMatches = icsData.match(/<(?:c|cal):calendar-data[^>]*><!\[CDATA\[([\s\S]*?)\]\]><\/(?:c|cal):calendar-data>/g);
      if (calendarDataMatches && calendarDataMatches.length > 0) {
        console.log(`Found ${calendarDataMatches.length} calendar-data sections with CDATA`);
        // Combine all calendar data into one iCal string
        icsData = calendarDataMatches.map(match => {
          const cdataMatch = match.match(/<!\[CDATA\[([\s\S]*?)\]\]>/);
          return cdataMatch ? cdataMatch[1] : '';
        }).join('\n');
      } else {
        // Try without CDATA - need to handle multiline content
        // Use a more flexible regex that handles content across multiple lines
        calendarDataMatches = icsData.match(/<(?:c|cal):calendar-data[^>]*>([\s\S]*?)<\/(?:c|cal):calendar-data>/g);
        if (calendarDataMatches && calendarDataMatches.length > 0) {
          console.log(`Found ${calendarDataMatches.length} calendar-data sections without CDATA`);
          icsData = calendarDataMatches.map(match => {
            // Extract content between > and </
            const contentMatch = match.match(/>([\s\S]*?)<\/(?:c|cal):calendar-data>/);
            if (contentMatch) {
              let content = contentMatch[1];
              // Unescape XML entities
              content = content.replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&').replace(/&quot;/g, '"');
              return content;
            }
            return '';
          }).join('\n');
        } else {
          console.log('No calendar-data found in XML response');
          console.log('Response preview:', icsData.substring(0, 1000));
          return [];
        }
      }
      
      // If we extracted multiple calendar items, parse each separately
      // Each calendar-data section is a complete VCALENDAR
      if (calendarDataMatches && calendarDataMatches.length > 1) {
        console.log(`Parsing ${calendarDataMatches.length} separate calendar items...`);
        const allVjournals = [];
        
        for (const match of calendarDataMatches) {
          let itemData;
          if (match.includes('CDATA')) {
            const cdataMatch = match.match(/<!\[CDATA\[([\s\S]*?)\]\]>/);
            itemData = cdataMatch ? cdataMatch[1] : '';
          } else {
            const contentMatch = match.match(/>([\s\S]*?)<\/(?:c|cal):calendar-data>/);
            if (contentMatch) {
              itemData = contentMatch[1].replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&').replace(/&quot;/g, '"');
            }
          }
          
          if (itemData && itemData.trim()) {
            try {
              const itemJcal = ICAL.parse(itemData.trim());
              const itemComp = new ICAL.Component(itemJcal);
              const itemVjournals = itemComp.getAllSubcomponents('vjournal');
              allVjournals.push(...itemVjournals);
            } catch (parseError) {
              console.warn('Failed to parse individual calendar item:', parseError.message);
            }
          }
        }
        
        console.log(`Found ${allVjournals.length} VJOURNAL components across all calendar items`);
        // Use the collected vjournals and continue with mapping below
        const vjournals = allVjournals;
        
        // Map vjournals to note objects (shared logic)
        // Note: getAllSubcomponents returns ICAL.Component instances, don't wrap them
        let notes = vjournals.map(vjournal => {
          const summary = vjournal.getFirstPropertyValue('summary') || '';
          const description = vjournal.getFirstPropertyValue('description') || '';
          const dtstart = vjournal.getFirstPropertyValue('dtstart');
          const uid = vjournal.getFirstPropertyValue('uid') || generateUID();
          
          return {
            uid: uid,
            summary: summary,
            description: description,
            created: dtstart ? new Date(dtstart.toJSDate()) : new Date(),
            source_id: defaultCalendar.id,
            source_name: defaultCalendar.name,
            source_color: defaultCalendar.color
          };
        });

        return notes.sort((a, b) => new Date(b.created) - new Date(a.created));
      }
      
      // Clean up the extracted iCal data (single calendar item)
      icsData = icsData.trim();
      console.log(`Extracted iCal data (${icsData.length} chars), first 200 chars:`, icsData.substring(0, 200));
    }
    
    console.log(`Fetched calendar data (${icsData.length} chars), looking for VJOURNAL components...`);
    
    let jcalData;
    try {
      jcalData = ICAL.parse(icsData);
    } catch (parseError) {
      // If parsing fails, return empty array (calendar might be empty or invalid)
      console.warn('Failed to parse calendar data, returning empty notes:', parseError.message);
      console.log('Response data preview:', icsData.substring(0, 500));
      return [];
    }
    
    const comp = new ICAL.Component(jcalData);
    const vjournals = comp.getAllSubcomponents('vjournal');
    console.log(`Found ${vjournals.length} VJOURNAL components in calendar`);

    // Note: getAllSubcomponents returns ICAL.Component instances, don't wrap them
    const notes = vjournals.map(vjournal => {
      const summary = vjournal.getFirstPropertyValue('summary') || '';
      const description = vjournal.getFirstPropertyValue('description') || '';
      const dtstart = vjournal.getFirstPropertyValue('dtstart');
      const lastModified = vjournal.getFirstPropertyValue('last-modified');
      const uid = vjournal.getFirstPropertyValue('uid') || generateUID();

      return {
        uid: uid,
        summary: summary,
        description: description,
        created: dtstart ? new Date(dtstart.toJSDate()) : new Date(),
        modified: lastModified ? new Date(lastModified.toJSDate()) : new Date(),
        source_id: defaultCalendar.id,
        source_name: defaultCalendar.name,
        source_color: defaultCalendar.color
      };
    });

    return notes.sort((a, b) => new Date(b.modified) - new Date(a.modified));
  } catch (error) {
    console.error('Error fetching notes:', error);
    reply.status(500).send({ error: 'Failed to fetch notes.' });
  }
});

// POST /api/caldav/notes - Create new note in default calendar
fastify.post('/api/caldav/notes', async (request, reply) => {
  try {
    const defaultCalendar = getDefaultCalendar();
    if (!defaultCalendar) {
      return reply.status(400).send({ error: 'No default calendar set. Please set a default calendar in Admin Panel.' });
    }

    const { summary, description } = request.body;
    if (!summary) {
      return reply.status(400).send({ error: 'Summary is required.' });
    }

    const uid = generateUID();
    const now = new Date();

    // Create VJOURNAL component
    const vjournal = new ICAL.Component('vjournal');
    vjournal.addPropertyWithValue('uid', uid);
    vjournal.addPropertyWithValue('dtstamp', ICAL.Time.fromJSDate(now));
    vjournal.addPropertyWithValue('dtstart', ICAL.Time.fromJSDate(now));
    vjournal.addPropertyWithValue('last-modified', ICAL.Time.fromJSDate(now));
    vjournal.addPropertyWithValue('summary', summary);
    if (description) {
      vjournal.addPropertyWithValue('description', description);
    }

    // Create calendar component with VJOURNAL
    const calComponent = new ICAL.Component('vcalendar');
    calComponent.addPropertyWithValue('version', '2.0');
    calComponent.addPropertyWithValue('prodid', '-//MindPalace//NONSGML v1.0//EN');
    calComponent.addSubcomponent(vjournal);
    const icsString = calComponent.toString();

    // PUT to CalDAV server
    const authHeader = getCalDAVAuthHeader(defaultCalendar);
    const calendarUrl = defaultCalendar.url.endsWith('/') ? defaultCalendar.url : `${defaultCalendar.url}/`;
    const noteUrl = `${calendarUrl}${uid}.ics`;
    
    console.log(`Creating note at: ${noteUrl}`);
    
    await axios.put(noteUrl, icsString, {
      headers: {
        'Authorization': authHeader,
        'Content-Type': 'text/calendar'
      },
      timeout: 15000
    });

    return { success: true, uid: uid, message: 'Note created successfully' };
  } catch (error) {
    console.error('Error creating note:', error);
    reply.status(500).send({ error: 'Failed to create note.' });
  }
});

// PUT /api/caldav/notes/:uid - Update note
fastify.put('/api/caldav/notes/:uid', async (request, reply) => {
  try {
    const { uid } = request.params;
    const defaultCalendar = getDefaultCalendar();
    if (!defaultCalendar) {
      return reply.status(400).send({ error: 'No default calendar set.' });
    }

    const { summary, description } = request.body;
    const authHeader = getCalDAVAuthHeader(defaultCalendar);

    // Fetch existing note
    const calendarUrl = defaultCalendar.url.endsWith('/') ? defaultCalendar.url : `${defaultCalendar.url}/`;
    const noteUrl = `${calendarUrl}${uid}.ics`;
    let existingNote;
    try {
      const response = await axios.get(noteUrl, {
        headers: { 'Authorization': authHeader },
        timeout: 15000
      });
      const jcalData = ICAL.parse(response.data);
      existingNote = new ICAL.Component(jcalData);
    } catch (error) {
      return reply.status(404).send({ error: 'Note not found.' });
    }

    // Update properties
    const vjournal = existingNote.getFirstSubcomponent('vjournal');
    if (summary !== undefined) vjournal.updatePropertyWithValue('summary', summary);
    if (description !== undefined) vjournal.updatePropertyWithValue('description', description);
    vjournal.updatePropertyWithValue('last-modified', ICAL.Time.fromJSDate(new Date()));

    // PUT updated note
    await axios.put(noteUrl, existingNote.toString(), {
      headers: {
        'Authorization': authHeader,
        'Content-Type': 'text/calendar'
      },
      timeout: 15000
    });

    return { success: true, message: 'Note updated successfully' };
  } catch (error) {
    console.error('Error updating note:', error);
    reply.status(500).send({ error: 'Failed to update note.' });
  }
});

// DELETE /api/caldav/notes/:uid - Delete note
fastify.delete('/api/caldav/notes/:uid', async (request, reply) => {
  try {
    const { uid } = request.params;
    const defaultCalendar = getDefaultCalendar();
    if (!defaultCalendar) {
      return reply.status(400).send({ error: 'No default calendar set.' });
    }

    const authHeader = getCalDAVAuthHeader(defaultCalendar);
    const calendarUrl = defaultCalendar.url.endsWith('/') ? defaultCalendar.url : `${defaultCalendar.url}/`;
    const noteUrl = `${calendarUrl}${uid}.ics`;

    await axios.delete(noteUrl, {
      headers: { 'Authorization': authHeader },
      timeout: 15000
    });

    return { success: true, message: 'Note deleted successfully' };
  } catch (error) {
    console.error('Error deleting note:', error);
    if (error.response && error.response.status === 404) {
      return reply.status(404).send({ error: 'Note not found.' });
    }
    reply.status(500).send({ error: 'Failed to delete note.' });
  }
});

// ============================================
// ALARM ENDPOINTS
// ============================================

// GET /api/alarms - Get all alarms (global)
fastify.get('/api/alarms', async (request, reply) => {
  try {
    const alarms = db.prepare('SELECT * FROM alarms ORDER BY time, created_at').all();
    return alarms.map(alarm => ({
      ...alarm,
      enabled: Boolean(alarm.enabled),
      sound_enabled: Boolean(alarm.sound_enabled),
      notification_enabled: Boolean(alarm.notification_enabled),
      visual_alert_enabled: Boolean(alarm.visual_alert_enabled),
      repeat_days: alarm.repeat_days ? JSON.parse(alarm.repeat_days) : null
    }));
  } catch (error) {
    console.error('Error fetching alarms:', error);
    reply.status(500).send({ error: 'Failed to fetch alarms.' });
  }
});

// POST /api/alarms - Create new alarm
fastify.post('/api/alarms', async (request, reply) => {
  try {
    const { title, time, repeat_type, repeat_days, enabled, sound_enabled, notification_enabled, visual_alert_enabled } = request.body;
    
    if (!title || !time) {
      return reply.status(400).send({ error: 'Title and time are required.' });
    }

    const repeatDaysJson = repeat_days ? JSON.stringify(repeat_days) : null;
    const stmt = db.prepare(`
      INSERT INTO alarms (title, time, repeat_type, repeat_days, enabled, sound_enabled, notification_enabled, visual_alert_enabled)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `);
    const info = stmt.run(
      title,
      time,
      repeat_type || 'none',
      repeatDaysJson,
      enabled !== undefined ? (enabled ? 1 : 0) : 1,
      sound_enabled !== undefined ? (sound_enabled ? 1 : 0) : 1,
      notification_enabled !== undefined ? (notification_enabled ? 1 : 0) : 1,
      visual_alert_enabled !== undefined ? (visual_alert_enabled ? 1 : 0) : 1
    );

    return { success: true, id: info.lastInsertRowid, message: 'Alarm created successfully' };
  } catch (error) {
    console.error('Error creating alarm:', error);
    reply.status(500).send({ error: 'Failed to create alarm.' });
  }
});

// PUT /api/alarms/:id - Update alarm
fastify.put('/api/alarms/:id', async (request, reply) => {
  try {
    const { id } = request.params;
    const { title, time, repeat_type, repeat_days, enabled, sound_enabled, notification_enabled, visual_alert_enabled } = request.body;

    const existing = db.prepare('SELECT * FROM alarms WHERE id = ?').get(id);
    if (!existing) {
      return reply.status(404).send({ error: 'Alarm not found.' });
    }

    const repeatDaysJson = repeat_days !== undefined ? (repeat_days ? JSON.stringify(repeat_days) : null) : existing.repeat_days;
    const stmt = db.prepare(`
      UPDATE alarms 
      SET title = COALESCE(?, title),
          time = COALESCE(?, time),
          repeat_type = COALESCE(?, repeat_type),
          repeat_days = ?,
          enabled = COALESCE(?, enabled),
          sound_enabled = COALESCE(?, sound_enabled),
          notification_enabled = COALESCE(?, notification_enabled),
          visual_alert_enabled = COALESCE(?, visual_alert_enabled)
      WHERE id = ?
    `);
    stmt.run(
      title || null,
      time || null,
      repeat_type || null,
      repeatDaysJson,
      enabled !== undefined ? (enabled ? 1 : 0) : null,
      sound_enabled !== undefined ? (sound_enabled ? 1 : 0) : null,
      notification_enabled !== undefined ? (notification_enabled ? 1 : 0) : null,
      visual_alert_enabled !== undefined ? (visual_alert_enabled ? 1 : 0) : null,
      id
    );

    return { success: true, message: 'Alarm updated successfully' };
  } catch (error) {
    console.error('Error updating alarm:', error);
    reply.status(500).send({ error: 'Failed to update alarm.' });
  }
});

// DELETE /api/alarms/:id - Delete alarm
fastify.delete('/api/alarms/:id', async (request, reply) => {
  try {
    const { id } = request.params;
    const existing = db.prepare('SELECT * FROM alarms WHERE id = ?').get(id);
    if (!existing) {
      return reply.status(404).send({ error: 'Alarm not found.' });
    }

    db.prepare('DELETE FROM alarms WHERE id = ?').run(id);
    return { success: true, message: 'Alarm deleted successfully' };
  } catch (error) {
    console.error('Error deleting alarm:', error);
    reply.status(500).send({ error: 'Failed to delete alarm.' });
  }
});

// POST /api/alarms/:id/toggle - Toggle alarm enabled state
fastify.post('/api/alarms/:id/toggle', async (request, reply) => {
  try {
    const { id } = request.params;
    const existing = db.prepare('SELECT * FROM alarms WHERE id = ?').get(id);
    if (!existing) {
      return reply.status(404).send({ error: 'Alarm not found.' });
    }

    const newEnabled = existing.enabled ? 0 : 1;
    db.prepare('UPDATE alarms SET enabled = ? WHERE id = ?').run(newEnabled, id);
    return { success: true, enabled: Boolean(newEnabled), message: 'Alarm toggled successfully' };
  } catch (error) {
    console.error('Error toggling alarm:', error);
    reply.status(500).send({ error: 'Failed to toggle alarm.' });
  }
});

// PUT /api/alarms/:id/triggered - Update last_triggered timestamp
fastify.put('/api/alarms/:id/triggered', async (request, reply) => {
  try {
    const { id } = request.params;
    const now = new Date().toISOString();
    db.prepare('UPDATE alarms SET last_triggered = ? WHERE id = ?').run(now, id);
    return { success: true, message: 'Alarm trigger timestamp updated' };
  } catch (error) {
    console.error('Error updating alarm trigger:', error);
    reply.status(500).send({ error: 'Failed to update alarm trigger.' });
  }
});

// ============================================
// HOUSE RULES ENDPOINTS
// ============================================

// GET /api/house-rules - Get all house rules (ordered by order_index)
fastify.get('/api/house-rules', async (request, reply) => {
  try {
    const rules = db.prepare('SELECT * FROM house_rules ORDER BY order_index, id').all();
    return rules;
  } catch (error) {
    console.error('Error fetching house rules:', error);
    reply.status(500).send({ error: 'Failed to fetch house rules.' });
  }
});

// POST /api/house-rules - Create new rule (admin only)
fastify.post('/api/house-rules', async (request, reply) => {
  try {
    const pinError = requireAdminPIN(request, reply);
    if (pinError) return pinError;

    const { rule_text, order_index } = request.body;
    if (!rule_text) {
      return reply.status(400).send({ error: 'Rule text is required.' });
    }

    const maxOrder = db.prepare('SELECT MAX(order_index) as max FROM house_rules').get();
    const newOrderIndex = order_index !== undefined ? order_index : ((maxOrder?.max || -1) + 1);

    const stmt = db.prepare('INSERT INTO house_rules (rule_text, order_index) VALUES (?, ?)');
    const info = stmt.run(rule_text, newOrderIndex);

    return { success: true, id: info.lastInsertRowid, message: 'House rule created successfully' };
  } catch (error) {
    console.error('Error creating house rule:', error);
    reply.status(500).send({ error: 'Failed to create house rule.' });
  }
});

// PUT /api/house-rules/:id - Update rule (admin only)
fastify.put('/api/house-rules/:id', async (request, reply) => {
  try {
    const pinError = requireAdminPIN(request, reply);
    if (pinError) return pinError;

    const { id } = request.params;
    const { rule_text, order_index } = request.body;

    const existing = db.prepare('SELECT * FROM house_rules WHERE id = ?').get(id);
    if (!existing) {
      return reply.status(404).send({ error: 'House rule not found.' });
    }

    const now = new Date().toISOString();
    const stmt = db.prepare(`
      UPDATE house_rules 
      SET rule_text = COALESCE(?, rule_text),
          order_index = COALESCE(?, order_index),
          updated_at = ?
      WHERE id = ?
    `);
    stmt.run(rule_text || null, order_index !== undefined ? order_index : null, now, id);

    return { success: true, message: 'House rule updated successfully' };
  } catch (error) {
    console.error('Error updating house rule:', error);
    reply.status(500).send({ error: 'Failed to update house rule.' });
  }
});

// DELETE /api/house-rules/:id - Delete rule (admin only)
fastify.delete('/api/house-rules/:id', async (request, reply) => {
  try {
    const pinError = requireAdminPIN(request, reply);
    if (pinError) return pinError;

    const { id } = request.params;
    const existing = db.prepare('SELECT * FROM house_rules WHERE id = ?').get(id);
    if (!existing) {
      return reply.status(404).send({ error: 'House rule not found.' });
    }

    db.prepare('DELETE FROM house_rules WHERE id = ?').run(id);
    return { success: true, message: 'House rule deleted successfully' };
  } catch (error) {
    console.error('Error deleting house rule:', error);
    reply.status(500).send({ error: 'Failed to delete house rule.' });
  }
});

// PUT /api/house-rules/reorder - Reorder rules (admin only)
fastify.put('/api/house-rules/reorder', async (request, reply) => {
  try {
    const pinError = requireAdminPIN(request, reply);
    if (pinError) return pinError;

    const { ruleIds } = request.body; // Array of rule IDs in new order
    if (!Array.isArray(ruleIds)) {
      return reply.status(400).send({ error: 'ruleIds must be an array.' });
    }

    const updateStmt = db.prepare('UPDATE house_rules SET order_index = ? WHERE id = ?');
    ruleIds.forEach((id, index) => {
      updateStmt.run(index, id);
    });

    return { success: true, message: 'House rules reordered successfully' };
  } catch (error) {
    console.error('Error reordering house rules:', error);
    reply.status(500).send({ error: 'Failed to reorder house rules.' });
  }
});

// ============================================
// MARBLE ENDPOINTS
// ============================================

// GET /api/marbles - Get all users' marble counts (only tracked users)
fastify.get('/api/marbles', async (request, reply) => {
  try {
    const marbles = db.prepare(`
      SELECT m.*, u.username 
      FROM marbles m
      LEFT JOIN users u ON m.user_id = u.id
      WHERE m.track_marbles = 1
      ORDER BY u.username
    `).all();
    return marbles;
  } catch (error) {
    console.error('Error fetching marbles:', error);
    reply.status(500).send({ error: 'Failed to fetch marbles.' });
  }
});

// GET /api/marbles/all - Get all users with tracking status (admin only, for admin panel)
fastify.get('/api/marbles/all', async (request, reply) => {
  try {
    const pinError = requireAdminPIN(request, reply);
    if (pinError) return pinError;

    const users = db.prepare('SELECT id, username FROM users ORDER BY username').all();
    const marbles = db.prepare(`
      SELECT m.*, u.username 
      FROM marbles m
      LEFT JOIN users u ON m.user_id = u.id
    `).all();
    
    const marbleMap = new Map(marbles.map(m => [m.user_id, m]));
    
    return users.map(user => ({
      user_id: user.id,
      username: user.username,
      track_marbles: marbleMap.has(user.id) ? Boolean(marbleMap.get(user.id).track_marbles) : false,
      count: marbleMap.has(user.id) ? marbleMap.get(user.id).count : 0
    }));
  } catch (error) {
    console.error('Error fetching all marbles:', error);
    reply.status(500).send({ error: 'Failed to fetch marbles.' });
  }
});

// POST /api/marbles/:userId/toggle-tracking - Toggle marble tracking for a user (admin only)
fastify.post('/api/marbles/:userId/toggle-tracking', async (request, reply) => {
  try {
    const pinError = requireAdminPIN(request, reply);
    if (pinError) return pinError;

    const { userId } = request.params;
    
    let marble = db.prepare('SELECT * FROM marbles WHERE user_id = ?').get(userId);
    if (!marble) {
      // Create new record with tracking enabled
      db.prepare('INSERT INTO marbles (user_id, count, track_marbles) VALUES (?, ?, ?)').run(userId, 0, 1);
      return { success: true, track_marbles: true, message: 'Marble tracking enabled' };
    } else {
      // Toggle tracking
      const newTrackValue = marble.track_marbles ? 0 : 1;
      db.prepare('UPDATE marbles SET track_marbles = ? WHERE user_id = ?').run(newTrackValue, userId);
      return { success: true, track_marbles: Boolean(newTrackValue), message: `Marble tracking ${newTrackValue ? 'enabled' : 'disabled'}` };
    }
  } catch (error) {
    console.error('Error toggling marble tracking:', error);
    reply.status(500).send({ error: 'Failed to toggle marble tracking.' });
  }
});

// GET /api/marbles/:userId - Get specific user's marble count
fastify.get('/api/marbles/:userId', async (request, reply) => {
  try {
    const { userId } = request.params;
    const marble = db.prepare('SELECT * FROM marbles WHERE user_id = ?').get(userId);
    if (!marble) {
      // Return default if user has no marbles record
      return { user_id: parseInt(userId), count: 0 };
    }
    return marble;
  } catch (error) {
    console.error('Error fetching user marbles:', error);
    reply.status(500).send({ error: 'Failed to fetch user marbles.' });
  }
});

// POST /api/marbles/:userId/earn - Award daily marbles (automated or manual, only for tracked users)
fastify.post('/api/marbles/:userId/earn', async (request, reply) => {
  try {
    const { userId } = request.params;
    const { amount } = request.body;

    // Check if user is tracked
    const marble = db.prepare('SELECT * FROM marbles WHERE user_id = ? AND track_marbles = 1').get(userId);
    if (!marble) {
      return reply.status(400).send({ error: 'User is not tracked for marbles. Enable tracking in admin panel first.' });
    }

    const incrementSetting = db.prepare('SELECT value FROM settings WHERE key = ?').get('MARBLE_DAILY_INCREMENT');
    const incrementAmount = amount !== undefined ? amount : parseInt(incrementSetting?.value || '3');

    const newCount = marble.count + incrementAmount;
    db.prepare('UPDATE marbles SET count = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?').run(newCount, userId);
    marble.count = newCount;

    // Add to history
    db.prepare(`
      INSERT INTO marble_history (user_id, change_amount, change_type, reason)
      VALUES (?, ?, ?, ?)
    `).run(userId, incrementAmount, 'earned', request.body.reason || 'Daily increment');

    return { success: true, marble: { ...marble, count: newCount }, message: 'Marbles awarded successfully' };
  } catch (error) {
    console.error('Error awarding marbles:', error);
    reply.status(500).send({ error: 'Failed to award marbles.' });
  }
});

// POST /api/marbles/:userId/remove - Remove marbles (admin only, requires reason)
fastify.post('/api/marbles/:userId/remove', async (request, reply) => {
  try {
    const pinError = requireAdminPIN(request, reply);
    if (pinError) return pinError;

    const { userId } = request.params;
    const { amount, reason } = request.body;

    if (!amount || amount <= 0) {
      return reply.status(400).send({ error: 'Amount must be a positive number.' });
    }
    if (!reason) {
      return reply.status(400).send({ error: 'Reason is required for removing marbles.' });
    }

    // Get marble record
    let marble = db.prepare('SELECT * FROM marbles WHERE user_id = ?').get(userId);
    if (!marble) {
      return reply.status(404).send({ error: 'User has no marbles record.' });
    }

    const newCount = Math.max(0, marble.count - amount);
    db.prepare('UPDATE marbles SET count = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?').run(newCount, userId);
    marble.count = newCount;

    // Add to history
    db.prepare(`
      INSERT INTO marble_history (user_id, change_amount, change_type, reason)
      VALUES (?, ?, ?, ?)
    `).run(userId, -amount, 'removed', reason);

    return { success: true, marble, message: 'Marbles removed successfully' };
  } catch (error) {
    console.error('Error removing marbles:', error);
    reply.status(500).send({ error: 'Failed to remove marbles.' });
  }
});

// POST /api/marbles/:userId/override - Override marble count (admin only, requires reason)
fastify.post('/api/marbles/:userId/override', async (request, reply) => {
  try {
    const pinError = requireAdminPIN(request, reply);
    if (pinError) return pinError;

    const { userId } = request.params;
    const { count, reason } = request.body;

    if (count === undefined || count === null || count < 0) {
      return reply.status(400).send({ error: 'Count must be a non-negative number.' });
    }
    if (!reason) {
      return reply.status(400).send({ error: 'Reason is required for overriding marbles.' });
    }

    // Get marble record
    let marble = db.prepare('SELECT * FROM marbles WHERE user_id = ?').get(userId);
    if (!marble) {
      return reply.status(404).send({ error: 'User has no marbles record.' });
    }

    const oldCount = marble.count;
    const newCount = Math.max(0, parseInt(count));
    const changeAmount = newCount - oldCount;

    db.prepare('UPDATE marbles SET count = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?').run(newCount, userId);
    marble.count = newCount;

    // Add to history
    db.prepare(`
      INSERT INTO marble_history (user_id, change_amount, change_type, reason)
      VALUES (?, ?, ?, ?)
    `).run(userId, changeAmount, 'manual', reason);

    return { success: true, marble, message: 'Marble count overridden successfully' };
  } catch (error) {
    console.error('Error overriding marbles:', error);
    reply.status(500).send({ error: 'Failed to override marbles.' });
  }
});

// GET /api/marbles/:userId/history - Get marble history for user
fastify.get('/api/marbles/:userId/history', async (request, reply) => {
  try {
    const { userId } = request.params;
    const { limit = 50 } = request.query;
    const history = db.prepare(`
      SELECT * FROM marble_history 
      WHERE user_id = ? 
      ORDER BY created_at DESC 
      LIMIT ?
    `).all(userId, limit);
    return history;
  } catch (error) {
    console.error('Error fetching marble history:', error);
    reply.status(500).send({ error: 'Failed to fetch marble history.' });
  }
});

// GET /api/marbles/settings - Get marble settings
fastify.get('/api/marbles/settings', async (request, reply) => {
  try {
    const incrementSetting = db.prepare('SELECT value FROM settings WHERE key = ?').get('MARBLE_DAILY_INCREMENT');
    return {
      daily_increment: parseInt(incrementSetting?.value || '3')
    };
  } catch (error) {
    console.error('Error fetching marble settings:', error);
    reply.status(500).send({ error: 'Failed to fetch marble settings.' });
  }
});

// PUT /api/marbles/settings - Update marble settings (admin only)
fastify.put('/api/marbles/settings', async (request, reply) => {
  try {
    const pinError = requireAdminPIN(request, reply);
    if (pinError) return pinError;

    const { daily_increment } = request.body;
    if (daily_increment === undefined || daily_increment < 0) {
      return reply.status(400).send({ error: 'Daily increment must be a non-negative number.' });
    }

    db.prepare('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)').run('MARBLE_DAILY_INCREMENT', String(daily_increment));
    return { success: true, message: 'Marble settings updated successfully' };
  } catch (error) {
    console.error('Error updating marble settings:', error);
    reply.status(500).send({ error: 'Failed to update marble settings.' });
  }
});

// POST /api/caldav/events - Create new event
fastify.post('/api/caldav/events', async (request, reply) => {
  try {
    const { calendar_id, summary, start, end, description, location } = request.body;
    if (!calendar_id || !summary || !start) {
      return reply.status(400).send({ error: 'Calendar ID, summary, and start date are required.' });
    }

    const calendar = db.prepare('SELECT * FROM calendar_sources WHERE id = ? AND enabled = 1 AND type = ?').get(calendar_id, 'CalDAV');
    if (!calendar) {
      return reply.status(404).send({ error: 'Calendar not found or not enabled.' });
    }

    const uid = generateUID();
    const now = new Date();

    // Create VEVENT component
    const vevent = new ICAL.Component('vevent');
    vevent.addPropertyWithValue('uid', uid);
    vevent.addPropertyWithValue('dtstamp', ICAL.Time.fromJSDate(now));
    vevent.addPropertyWithValue('dtstart', ICAL.Time.fromJSDate(new Date(start)));
    if (end) {
      vevent.addPropertyWithValue('dtend', ICAL.Time.fromJSDate(new Date(end)));
    }
    vevent.addPropertyWithValue('summary', summary);
    if (description) {
      vevent.addPropertyWithValue('description', description);
    }
    if (location) {
      vevent.addPropertyWithValue('location', location);
    }

    // Create calendar component with VEVENT
    const calComponent = new ICAL.Component('vcalendar');
    calComponent.addPropertyWithValue('version', '2.0');
    calComponent.addPropertyWithValue('prodid', '-//MindPalace//NONSGML v1.0//EN');
    calComponent.addSubcomponent(vevent);
    const icsString = calComponent.toString();

    // PUT to CalDAV server
    const authHeader = getCalDAVAuthHeader(calendar);
    const calendarUrl = calendar.url.endsWith('/') ? calendar.url : `${calendar.url}/`;
    const eventUrl = `${calendarUrl}${uid}.ics`;
    
    console.log(`Creating event at: ${eventUrl}`);
    
    await axios.put(eventUrl, icsString, {
      headers: {
        'Authorization': authHeader,
        'Content-Type': 'text/calendar'
      },
      timeout: 15000
    });

    return { success: true, uid: uid, message: 'Event created successfully' };
  } catch (error) {
    console.error('Error creating event:', error);
    if (error.response) {
      const status = error.response.status;
      const errorData = error.response.data;
      let errorMessage = `CalDAV server returned ${status}: `;
      
      if (typeof errorData === 'string' && errorData.includes('<s:message>')) {
        const match = errorData.match(/<s:message>(.*?)<\/s:message>/);
        if (match) {
          errorMessage += match[1];
        } else {
          errorMessage += errorData.substring(0, 200);
        }
      } else {
        errorMessage += error.response.statusText || 'Unknown error';
      }
      
      return reply.status(status).send({ 
        error: errorMessage,
        details: `Failed to create event at ${error.config?.url || 'unknown URL'}. Please check your calendar URL and permissions.`
      });
    }
    reply.status(500).send({ error: 'Failed to create event.', details: error.message });
  }
});

// PUT /api/caldav/events/:uid - Update event
fastify.put('/api/caldav/events/:uid', async (request, reply) => {
  try {
    const { uid } = request.params;
    const { calendar_id, summary, start, end, description, location } = request.body;
    
    if (!calendar_id) {
      return reply.status(400).send({ error: 'Calendar ID is required.' });
    }

    const calendar = db.prepare('SELECT * FROM calendar_sources WHERE id = ? AND enabled = 1 AND type = ?').get(calendar_id, 'CalDAV');
    if (!calendar) {
      return reply.status(404).send({ error: 'Calendar not found.' });
    }

    const authHeader = getCalDAVAuthHeader(calendar);
    const calendarUrl = calendar.url.endsWith('/') ? calendar.url : `${calendar.url}/`;
    const eventUrl = `${calendarUrl}${uid}.ics`;

    // Fetch existing event
    let existingEvent;
    try {
      const response = await axios.get(eventUrl, {
        headers: { 'Authorization': authHeader },
        timeout: 15000
      });
      const jcalData = ICAL.parse(response.data);
      existingEvent = new ICAL.Component(jcalData);
    } catch (error) {
      return reply.status(404).send({ error: 'Event not found.' });
    }

    // Update properties
    const vevent = existingEvent.getFirstSubcomponent('vevent');
    if (summary !== undefined) vevent.updatePropertyWithValue('summary', summary);
    if (start !== undefined) vevent.updatePropertyWithValue('dtstart', ICAL.Time.fromJSDate(new Date(start)));
    if (end !== undefined) {
      if (end) {
        vevent.updatePropertyWithValue('dtend', ICAL.Time.fromJSDate(new Date(end)));
      } else {
        vevent.removeProperty('dtend');
      }
    }
    if (description !== undefined) vevent.updatePropertyWithValue('description', description);
    if (location !== undefined) {
      if (location) {
        vevent.updatePropertyWithValue('location', location);
      } else {
        vevent.removeProperty('location');
      }
    }

    // PUT updated event
    await axios.put(eventUrl, existingEvent.toString(), {
      headers: {
        'Authorization': authHeader,
        'Content-Type': 'text/calendar'
      },
      timeout: 15000
    });

    return { success: true, message: 'Event updated successfully' };
  } catch (error) {
    console.error('Error updating event:', error);
    reply.status(500).send({ error: 'Failed to update event.' });
  }
});

// DELETE /api/caldav/events/:uid - Delete event
fastify.delete('/api/caldav/events/:uid', async (request, reply) => {
  try {
    const { uid } = request.params;
    const { calendar_id } = request.query;
    
    if (!calendar_id) {
      return reply.status(400).send({ error: 'Calendar ID is required.' });
    }

    const calendar = db.prepare('SELECT * FROM calendar_sources WHERE id = ? AND enabled = 1 AND type = ?').get(calendar_id, 'CalDAV');
    if (!calendar) {
      return reply.status(404).send({ error: 'Calendar not found.' });
    }

    const authHeader = getCalDAVAuthHeader(calendar);
    const calendarUrl = calendar.url.endsWith('/') ? calendar.url : `${calendar.url}/`;
    const eventUrl = `${calendarUrl}${uid}.ics`;

    await axios.delete(eventUrl, {
      headers: { 'Authorization': authHeader },
      timeout: 15000
    });

    return { success: true, message: 'Event deleted successfully' };
  } catch (error) {
    console.error('Error deleting event:', error);
    if (error.response && error.response.status === 404) {
      return reply.status(404).send({ error: 'Event not found.' });
    }
    reply.status(500).send({ error: 'Failed to delete event.' });
  }
});


// Photo sources routes
fastify.get('/api/photo-sources', async (request, reply) => {
  try {
    const rows = db.prepare('SELECT id, name, type, url, album_id, enabled, sort_order, created_at FROM photo_sources ORDER BY sort_order, id').all();
    return rows;
  } catch (error) {
    console.error('Error fetching photo sources:', error);
    reply.status(500).send({ error: 'Failed to fetch photo sources' });
  }
});

fastify.post('/api/photo-sources', async (request, reply) => {
  const { name, type, url, api_key, username, password, album_id, refresh_token } = request.body;
  if (!name || !type) {
    return reply.status(400).send({ error: 'Name and type are required.' });
  }
  if (!['Immich', 'GooglePhotos'].includes(type)) {
    return reply.status(400).send({ error: 'Type must be either Immich or GooglePhotos.' });
  }
  try {
    const encryptedApiKey = api_key ? encryptPassword(api_key) : null;
    const encryptedPassword = password ? encryptPassword(password) : null;
    const encryptedRefreshToken = refresh_token ? encryptPassword(refresh_token) : null;
    const maxOrder = db.prepare('SELECT MAX(sort_order) as max FROM photo_sources').get();
    const nextOrder = (maxOrder.max || 0) + 1;

    const stmt = db.prepare(`
      INSERT INTO photo_sources (name, type, url, api_key, username, password, album_id, refresh_token, enabled, sort_order)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    const info = stmt.run(name, type, url || null, encryptedApiKey, username || null, encryptedPassword, album_id || null, encryptedRefreshToken, 1, nextOrder);
    return { id: info.lastInsertRowid, success: true };
  } catch (error) {
    console.error('Error adding photo source:', error);
    reply.status(500).send({ error: 'Failed to add photo source' });
  }
});

fastify.patch('/api/photo-sources/:id', async (request, reply) => {
  const { id } = request.params;
  const { name, type, url, api_key, username, password, album_id, refresh_token, enabled } = request.body;

  try {
    const existing = db.prepare('SELECT * FROM photo_sources WHERE id = ?').get(id);
    if (!existing) {
      return reply.status(404).send({ error: 'Photo source not found' });
    }

    // Check if album_id changed - if so, clean cache for old album
    if (album_id !== undefined && album_id !== existing.album_id) {
      await cleanSourceCache(id, existing.album_id);
      if (album_id) {
        await cleanSourceCache(id, album_id);
      }
    }

    // Check if source type or URL changed - clean entire source cache
    if ((type !== undefined && type !== existing.type) || 
        (url !== undefined && url !== existing.url)) {
      await cleanSourceCache(id);
    }

    const updateFields = [];
    const updateValues = [];

    if (name !== undefined) { updateFields.push('name = ?'); updateValues.push(name); }
    if (type !== undefined) {
      if (!['Immich', 'GooglePhotos'].includes(type)) {
        return reply.status(400).send({ error: 'Type must be either Immich or GooglePhotos.' });
      }
      updateFields.push('type = ?');
      updateValues.push(type);
    }
    if (url !== undefined) { updateFields.push('url = ?'); updateValues.push(url || null); }
    if (api_key !== undefined && api_key !== '') {
      const encryptedApiKey = encryptPassword(api_key);
      updateFields.push('api_key = ?');
      updateValues.push(encryptedApiKey);
    }
    if (username !== undefined) { updateFields.push('username = ?'); updateValues.push(username || null); }
    if (password !== undefined && password !== '') {
      const encryptedPassword = encryptPassword(password);
      updateFields.push('password = ?');
      updateValues.push(encryptedPassword);
    }
    if (album_id !== undefined) { updateFields.push('album_id = ?'); updateValues.push(album_id || null); }
    if (refresh_token !== undefined && refresh_token !== '') {
      const encryptedRefreshToken = encryptPassword(refresh_token);
      updateFields.push('refresh_token = ?');
      updateValues.push(encryptedRefreshToken);
    }
    if (enabled !== undefined) { updateFields.push('enabled = ?'); updateValues.push(enabled ? 1 : 0); }

    if (updateFields.length === 0) {
      return reply.status(400).send({ error: 'No fields to update' });
    }

    updateValues.push(id);
    const stmt = db.prepare(`UPDATE photo_sources SET ${updateFields.join(', ')} WHERE id = ?`);
    const info = stmt.run(...updateValues);

    if (info.changes === 0) {
      return reply.status(404).send({ error: 'Photo source not found' });
    }
    return { success: true, message: 'Photo source updated successfully' };
  } catch (error) {
    console.error('Error updating photo source:', error);
    reply.status(500).send({ error: 'Failed to update photo source' });
  }
});

fastify.delete('/api/photo-sources/:id', async (request, reply) => {
  const { id } = request.params;
  try {
    // Clean cache before deleting
    await cleanSourceCache(id);
    
    const stmt = db.prepare('DELETE FROM photo_sources WHERE id = ?');
    const info = stmt.run(id);
    if (info.changes === 0) {
      return reply.status(404).send({ error: 'Photo source not found' });
    }
    return { success: true, message: 'Photo source deleted successfully' };
  } catch (error) {
    console.error('Error deleting photo source:', error);
    reply.status(500).send({ error: 'Failed to delete photo source' });
  }
});

fastify.post('/api/photo-sources/:id/test', async (request, reply) => {
  const { id } = request.params;
  try {
    const source = db.prepare('SELECT * FROM photo_sources WHERE id = ?').get(id);
    if (!source) {
      return reply.status(404).send({ error: 'Photo source not found' });
    }

    if (source.type === 'Immich') {
      const decryptedApiKey = decryptPassword(source.api_key);
      const response = await axios.post(`${source.url}/api/search/random`,
        { size: 1 },
        {
          headers: {
            'x-api-key': decryptedApiKey,
            'Content-Type': 'application/json',
            'Accept': 'application/json'
          },
          timeout: 10000
        }
      );
      return { success: true, assetCount: response.data.length || 0, message: 'Immich connection successful' };
    } else if (source.type === 'GooglePhotos') {
      const decryptedRefreshToken = decryptPassword(source.refresh_token);
      return { success: true, message: 'Google Photos configuration saved (connection test not yet implemented)' };
    }
  } catch (error) {
    console.error('Error testing photo source:', error.message);
    if (error.response) {
      console.error(`Response status: ${error.response.status}`);
      console.error(`Response data:`, error.response.data);
    }
    return reply.status(400).send({
      success: false,
      error: 'Failed to connect to photo source',
      details: error.message,
      responseStatus: error.response?.status,
      responseData: error.response?.data
    });
  }
});

// Proxy endpoint to serve Immich images with authentication
// Handle OPTIONS preflight for photo-proxy
fastify.options('/api/photo-proxy/:sourceId/:assetId', async (request, reply) => {
  reply.header('Access-Control-Allow-Origin', '*');
  reply.header('Access-Control-Allow-Methods', 'GET, OPTIONS');
  reply.header('Access-Control-Allow-Headers', 'Content-Type');
  reply.header('Access-Control-Max-Age', '86400');
  return reply.status(204).send();
});

fastify.get('/api/photo-proxy/:sourceId/:assetId', async (request, reply) => {
  const { sourceId, assetId } = request.params;
  const { size = 'preview' } = request.query;
  
  console.log(`[PHOTO-PROXY] Request received: sourceId=${sourceId}, assetId=${assetId}, size=${size}`);

  try {
    const source = db.prepare('SELECT * FROM photo_sources WHERE id = ?').get(sourceId);
    if (!source) {
      console.error(`[PHOTO-PROXY] Photo source not found: ${sourceId}`);
      reply.header('Access-Control-Allow-Origin', '*');
      reply.header('Access-Control-Allow-Methods', 'GET, OPTIONS');
      return reply.status(404).send({ error: 'Photo source not found' });
    }

    if (!source.enabled) {
      console.error(`[PHOTO-PROXY] Photo source is disabled: ${sourceId}`);
      reply.header('Access-Control-Allow-Origin', '*');
      reply.header('Access-Control-Allow-Methods', 'GET, OPTIONS');
      return reply.status(403).send({ error: 'Photo source is disabled' });
    }

    // Check cache first
    const albumId = source.album_id || null;
    const cachePath = getCachePath(sourceId, albumId, assetId, size);
    
    try {
      const cachedData = await fs.readFile(cachePath);
      console.log(`[PHOTO-PROXY] Serving from cache: ${cachePath}`);
      
      reply.header('Access-Control-Allow-Origin', '*');
      reply.header('Access-Control-Allow-Methods', 'GET, OPTIONS');
      reply.header('Access-Control-Allow-Headers', 'Content-Type');
      reply.header('Access-Control-Expose-Headers', 'Content-Type');
      reply.header('Content-Type', 'image/jpeg');
      reply.header('Cache-Control', 'public, max-age=86400');
      reply.header('X-Cache', 'HIT');
      
      return reply.send(cachedData);
    } catch (cacheError) {
      // Cache miss - fetch from source
      console.log(`[PHOTO-PROXY] Cache miss, fetching from source: ${cachePath}`);
    }

    const decryptedApiKey = decryptPassword(source.api_key);
    const immichUrl = `${source.url}/api/assets/${assetId}/thumbnail`;
    console.log(`[PHOTO-PROXY] Fetching from Immich: ${immichUrl} (size: ${size})`);
    
    const startTime = Date.now();
    const response = await axios.get(immichUrl, {
      headers: {
        'x-api-key': decryptedApiKey
      },
      params: { size },
      responseType: 'arraybuffer',
      timeout: 10000,
      validateStatus: (status) => status < 500
    });
    
    const fetchTime = Date.now() - startTime;
    console.log(`[PHOTO-PROXY] Immich response received in ${fetchTime}ms, status: ${response.status}, size: ${response.data?.length || 0} bytes`);

    if (response.status !== 200) {
      console.error(`[PHOTO-PROXY] Immich returned non-200 status: ${response.status}`);
      
      let errorMessage = `Immich returned status ${response.status}`;
      try {
        if (response.data && Buffer.isBuffer(response.data)) {
          const decoded = response.data.toString('utf-8');
          const parsed = JSON.parse(decoded);
          if (parsed.message) {
            errorMessage = parsed.message;
          }
          console.error(`[PHOTO-PROXY] Immich error details:`, parsed);
        }
      } catch (e) {
        // Ignore parsing errors
      }
      
      reply.header('Access-Control-Allow-Origin', '*');
      reply.header('Access-Control-Allow-Methods', 'GET, OPTIONS');
      return reply.status(response.status).send({ 
        error: errorMessage,
        status: response.status
      });
    }

    if (!response.data || response.data.length === 0) {
      console.error(`[PHOTO-PROXY] Empty response from Immich`);
      reply.header('Access-Control-Allow-Origin', '*');
      reply.header('Access-Control-Allow-Methods', 'GET, OPTIONS');
      return reply.status(500).send({ error: 'Empty response from photo source' });
    }

    // Save to cache
    try {
      const cacheDir = path.dirname(cachePath);
      await fs.mkdir(cacheDir, { recursive: true });
      await fs.writeFile(cachePath, Buffer.from(response.data));
      console.log(`[PHOTO-PROXY] Cached image: ${cachePath}`);
    } catch (cacheError) {
      console.error(`[PHOTO-PROXY] Error caching image:`, cacheError);
      // Continue even if caching fails
    }

    // Set CORS headers BEFORE setting other headers
    reply.header('Access-Control-Allow-Origin', '*');
    reply.header('Access-Control-Allow-Methods', 'GET, OPTIONS');
    reply.header('Access-Control-Allow-Headers', 'Content-Type');
    reply.header('Access-Control-Expose-Headers', 'Content-Type');
    reply.header('Content-Type', response.headers['content-type'] || 'image/jpeg');
    reply.header('Cache-Control', 'public, max-age=86400');
    reply.header('X-Cache', 'MISS');
    
    console.log(`[PHOTO-PROXY] Sending image response: ${response.data.length} bytes, Content-Type: ${response.headers['content-type'] || 'image/jpeg'}`);
    return reply.send(Buffer.from(response.data));
  } catch (error) {
    const errorTime = Date.now();
    console.error(`[PHOTO-PROXY] Error proxying photo (${errorTime}):`, error.message);
    console.error(`[PHOTO-PROXY] Error code:`, error.code);
    console.error(`[PHOTO-PROXY] Error stack:`, error.stack);
    
    if (error.response) {
      console.error(`[PHOTO-PROXY] Immich response status: ${error.response.status}`);
      console.error(`[PHOTO-PROXY] Immich response headers:`, error.response.headers);
      
      // Try to decode error message from response data
      let errorMessage = 'Unknown error';
      let errorDetails = null;
      try {
        if (error.response.data) {
          if (Buffer.isBuffer(error.response.data)) {
            const decoded = error.response.data.toString('utf-8');
            try {
              errorDetails = JSON.parse(decoded);
              errorMessage = errorDetails.message || decoded;
            } catch {
              errorMessage = decoded;
            }
          } else if (typeof error.response.data === 'string') {
            errorMessage = error.response.data;
          } else if (typeof error.response.data === 'object') {
            errorDetails = error.response.data;
            errorMessage = errorDetails.message || JSON.stringify(errorDetails);
          }
          console.error(`[PHOTO-PROXY] Immich error message:`, errorMessage);
          if (errorDetails) {
            console.error(`[PHOTO-PROXY] Immich error details:`, errorDetails);
          }
        }
      } catch (e) {
        console.error(`[PHOTO-PROXY] Could not decode error message:`, e);
      }
      
      // Provide helpful error message for common issues
      if (error.response.status === 403 && errorMessage.includes('asset.view')) {
        console.error(`[PHOTO-PROXY] API key missing required permissions. The API key needs 'asset.view' permission.`);
        console.error(`[PHOTO-PROXY] Please check your Immich API key permissions in the Immich admin panel.`);
      }
    }
    if (error.request) {
      console.error(`[PHOTO-PROXY] Request was made but no response received`);
      console.error(`[PHOTO-PROXY] Request config:`, {
        url: error.config?.url,
        method: error.config?.method,
        timeout: error.config?.timeout
      });
    }
    
    // Ensure CORS headers are set even on error
    reply.header('Access-Control-Allow-Origin', '*');
    reply.header('Access-Control-Allow-Methods', 'GET, OPTIONS');
    reply.header('Access-Control-Allow-Headers', 'Content-Type');
    
    // Return a proper error response
    const statusCode = error.response?.status || (error.code === 'ECONNABORTED' ? 504 : 500);
    return reply.status(statusCode).send({ 
      error: 'Failed to load photo',
      message: error.message,
      code: error.code
    });
  }
});

// Enhanced endpoint to fetch photos from multiple sources
fastify.get('/api/photo-items', async (request, reply) => {
  try {
    const sources = db.prepare('SELECT * FROM photo_sources WHERE enabled = 1 ORDER BY sort_order, id').all();

    if (sources.length === 0) {
      return [];
    }

    const fetchPromises = sources.map(async (source) => {
      try {
        if (source.type === 'Immich') {
          const decryptedApiKey = decryptPassword(source.api_key);

          // Use the search/random endpoint to get random assets
          const response = await axios.post(`${source.url}/api/search/random`,
            { size: 100 },
            {
              headers: {
                'x-api-key': decryptedApiKey,
                'Content-Type': 'application/json',
                'Accept': 'application/json'
              },
              timeout: 15000
            }
          );

          const assets = response.data || [];

          return assets.map(asset => ({
            id: asset.id,
            url: `/api/photo-proxy/${source.id}/${asset.id}?size=preview`,
            thumbnail: `/api/photo-proxy/${source.id}/${asset.id}?size=thumbnail`,
            type: asset.type,
            source_id: source.id,
            source_name: source.name,
            source_type: 'Immich'
          }));
        } else if (source.type === 'GooglePhotos') {
          return [];
        }
      } catch (error) {
        console.error(`Error fetching photos from source ${source.name}:`, error.message);
        if (error.response) {
          console.error(`Response status: ${error.response.status}`);
          console.error(`Response data:`, error.response.data);
        }
        return [];
      }
    });

    const results = await Promise.all(fetchPromises);
    const allPhotos = results.flat();

    // Shuffle photos
    for (let i = allPhotos.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [allPhotos[i], allPhotos[j]] = [allPhotos[j], allPhotos[i]];
    }

    return allPhotos;
  } catch (error) {
    console.error('Error fetching photos:', error);
    reply.status(500).send({ error: 'Failed to fetch photos.' });
  }
});

// Start server
const start = async () => {
  try {
    console.log('=== STARTING SERVER ===');
    console.log('Initializing database...');
    db = await initializeDatabase(); // Initialize db once here
    console.log('Database initialized successfully');
    
    console.log('Pruning and resetting chores...');
    await pruneAndResetChores(); // Call the pruning/reset function on startup
    console.log('Chores pruned and reset');
    
    const port = process.env.PORT || 5000;
    console.log(`Starting server on port ${port}...`);
    await fastify.listen({ port: port, host: '0.0.0.0' });
    console.log(`✓ Server running on port ${port}`);
    console.log(`✓ API available at http://0.0.0.0:${port}/api`);
    console.log('=== SERVER READY ===');
  } catch (err) {
    console.error('=== SERVER STARTUP ERROR ===');
    console.error('Error:', err);
    console.error('Error stack:', err.stack);
    console.error('Failed to start server');
    process.exit(1);
  }
};
start();
